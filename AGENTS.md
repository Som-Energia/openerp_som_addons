...
- Announce the active checkout or worktree before making edits.
- If creating a worktree, stop and get explicit approval before creating one when no appropriate worktree exists. Fetch `origin` first and create a new worktree branch from the current `origin/main`, never from a potentially stale local `main`.
