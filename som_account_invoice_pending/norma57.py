        ...
        for record in self.read(cursor, uid, ids, ['resource']):
            if record['resource']:
                ...
                if model_fact in ('account.invoice', 'giscedata.facturacio.factura'):
                    ...
                    if invoice_obj:
                        pending_state = invoice_obj.read(cursor, uid, id_fact, ['pending_state'])['pending_state'][1]
                        res[record['id']] = pending_state
        return res

    _columns = {
        'pending_state': fields.function(_pending_state, method=True, string='Estat pendent', type='string')
    }
