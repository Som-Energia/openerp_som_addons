...
class TestUpdatePendingStates(testing.OOTestCaseWithCursor):
    ...
    @mock.patch(
        "som_account_invoice_pending.update_pending_states.UpdatePendingStates.send_email"
    )
    ...
    def test__update_two_unpaid_invoice_waiting_for_annexII_same_contract(
        self, mock_function
    ):
        cursor = self.txn.cursor
        uid = self.txn.user
    ...
    def test__update_unpaid_invoice_waiting_for_annexIII_first_one_invoice(
        self, mock_function
    ):
        cursor = self.txn.cursor
        uid = self.txn.user
    ...
    @mock.patch(
        "som_account_invoice_pending.update_pending_states.UpdatePendingStates.send_sms"
    )
    ...
    def test__update_unpaid_invoice_waiting_for_48h_def(self, mock_sms, mock_mail):
        ...
        mock_sms.assert_called_once_with(
            ...
            uid,
            self.invoice_1_id,
            self.sms_48h_template_id,
            self.waiting_48h_def,
            {},
        )
    ...
    def test__update_two_unpaid_invoice_bs_waiting_for_48_raisesError(
        self, mock_sms, mock_mail
    ):
        cursor = self.txn.cursor
        uid = self.txn.user
        ...
        with mock.patch(
            "som_account_invoice_pending.update_pending_states.UpdatePendingStates.update_waiting_for_48h_active_contracts"
        ) as updateMock:
    ...
    def test__update_two_unpaid_invoice_dp_waiting_for_48_raisesError(
        self, mock_sms, mock_mail
    ):
        cursor = self.txn.cursor
        uid = self.txn.user
    ...
    def test__update_waiting_48h_invoice_bs_polissa_baixa_raisesError(
        self, mock_sms, mock_mail
    ):
        cursor = self.txn.cursor
        uid = self.txn.user
        ...
        with mock.patch(
            "som_account_invoice_pending.update_pending_states.UpdatePendingStates.update_waiting_for_annex_cancelled_contracts"
        ) as send_SMSMock:
    ...
    def test__update_waiting_48h_invoice_dp_polissa_baixa_raisesError(
        self, mock_sms, mock_mail
    ):
        cursor = self.txn.cursor
        uid = self.txn.user
    ...
    def test__update_two_unpaid_invoice_dp_waiting_for_annexIV_raisesError(
        ...
        with mock.patch(
            "som_account_invoice_pending.update_pending_states.UpdatePendingStates.update_waiting_for_annexIV_active_contracts"
        ) as send_SMSMock:
    ...
    def test__sms_callback_historize_annexIV(self, mock_sms, mock_mail):
        ...
        last_pending_id = min(
            fact_obj.read(cursor, uid, last_inv, ["pending_history_ids"])[
                "pending_history_ids"
            ]
        )
    ...
    def test__sms_callback_historize_48h_with_previous_observation(
        self, mock_sms, mock_mail
    ):
        cursor = self.txn.cursor
        uid = self.txn.user
        ...
        self.assertEqual(
            last_pending.observations,
            u"New message\nComunicació feta a través de la factura amb id:{}".format(
                first_inv
            ),
        )
    ...
    def test__update_waiting_for_annex_cancelled_contracts_baixa(
        self, mock_sms, mock_mail
    ):
        cursor = self.txn.cursor
        uid = self.txn.user
