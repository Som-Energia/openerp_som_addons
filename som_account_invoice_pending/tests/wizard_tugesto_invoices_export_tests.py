...
class TestWizardTugestoInvoicesExport(testing.OOTestCaseWithCursor):
    ...
    def _load_data_unpaid_invoices(self, cursor, uid, invoice_semid_list=[]):
        ...
        for index, res_id in enumerate(invoice_semid_list, start=1):
            ...
            invoice_id = self.fact_obj.read(cursor, uid, fact_id, ['invoice_id'])[
                'invoice_id'][0]

            if index == 1:
                contract_name = self.inv_obj.read(
                    cursor, uid, invoice_id, ['name'])['name']

            self.inv_obj.write(cursor, uid, invoice_id, {
                'name': contract_name,
            })
    ...
    def test_tugesto_invoices_export_no_active_ids__error(self):
        ...
        uid = self.uid
        self._load_demo_data(cursor, uid)
        invs_ids = [inv.invoice_id.id for inv in self.fact_obj.browse(
            cursor, uid, [self.invoice_1_id, self.invoice_2_id])]
    ...
    def test_tugesto_invoices_export_pending_state_updated__error(self):
        ...
        self.assertEqual(e.exception.value,
                         u"L'estat pendent d'alguna de les factures no és l'esperat")
