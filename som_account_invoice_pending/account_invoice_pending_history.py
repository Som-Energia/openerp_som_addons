...
class AccountInvoicePendingHistory(osv.osv):
    ...
    def historize(self, cursor, uid, ids, message=""):
        ...
        if not isinstance(ids, (tuple, list)):
            ids = [ids]
        for _id in ids:
            old_observations = self.read(cursor, uid, _id, ["observations"])[
                "observations"
            ]
    ...
    _columns = {
        ...
        "days_to_next_state": fields.integer(
            u"Dies pel següent canvi automàtic", required=False
        ),
    ...
    def powersms_create_callback(self, cursor, uid, ids, vals, context=None):
        ...
        for f_id in ids:
            ...
            if hist_id:
                hist_id = hist_id[0]
                hist_obj.write(
                    cursor, uid, hist_id, {"powersms_id": origin_ids.get(f_id, False)}
                )
        return True
                ...
                for f_id in ids:
                    ...
                    if sms_id:
                        hist_id = hist_obj.search(
                            cursor,
                            uid,
                            [("invoice_id", "=", inv_id), ("powersms_id", "=", sms_id)],
                        )
                        if hist_id:
