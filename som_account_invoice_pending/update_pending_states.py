...
class UpdatePendingStates(osv.osv_memory):
    ...
    def get_from_email(self, cursor, uid, template_id):
        ...
        if not email_from:
            email_from = account_obj.search(cursor, uid, [("name", "=", template_name)])[
                0
            ]
    ...
    def update_waiting_for_48h(self, cursor, uid, context=None):
        ...
        for factura_id in sorted(factura_dp_ids):
            ...
            try:
                ...
                logger.info(
                    "UNHANDLED ERROR updating invoice {} in update_waiting_for_48h: {}".format(
                        factura_id, e.message
                    )
                )
    ...
    def update_waiting_for_48h_active_contracts(
        ...
        try:
            ...
            email_params = dict(
                {"email_from": email_from, "template_id": mail_48h_template_id}
            )
            ...
            if ret_value == -1:
                ...
                if not related_invoice:
                    ...
                    pending_ids = fact_obj.read(
                        cursor, uid, factura_id, ["pending_history_ids"]
                    )["pending_history_ids"]
        ...
        for factura_id in sorted(factura_dp_ids):
            try:
                ...
                logger.info(
                    ...
                    "{} in update_waiting_for_annexIV: {}".format(factura_id, e.message)
                )
    ...
    def update_update_waiting_for_annexIII(
        ...
        for factura_id in factura_ids:
            ...
            if ret_value == -1:
                logger.info(
                    "ERROR: Sending Annex 3 first email to {} partner error.".format(
                ...
                logger.info(
                    "Sending Annex 3 first email to {} partner with result: {}".format(
                        invoice["partner_id"][1], ret_value
                    )
                )
    ...
    def update_waiting_for_annex_cancelled_contracts(
        ...
        try:
            ...
            email_params = dict(
                {"email_from": email_from, "template_id": n57_template_id}
            )
            ...
            if ret_value == -1:
                logger.info(
                    ...
                    " contract for {} invoice error.".format(factura_id)
                )
            else:
                try:
                    self.send_sms(
                    ...
                    fact_obj.write(
                        cursor, uid, factura_id, {"comment": new_comment + old_comment}
                    )
    ...
    def update_waiting_for_annexII_active_contracts(
    ...
    def update_waiting_for_annexIV_active_contracts(
        ...
        try:
            ...
            email_params = dict(
                {"email_from": email_from, "template_id": annex4_template_id}
            )
            ...
            if ret_value == -1:
                ...
                if not related_invoice:
                    ...
                    current_pending.historize(
                        message=(
                            u"Comunicació feta a través de la factura amb "
