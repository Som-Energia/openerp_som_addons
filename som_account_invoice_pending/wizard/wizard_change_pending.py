...
class WizardChangePending(osv.osv_memory):
    ...
    def onchange_new_pending(self, cursor, uid, ids, new_pending):
        ...
        pending_days = pending_obj.read(cursor, uid, new_pending, [
    ...
    def _calculate_pending_state_days(self, cursor, uid, new_pending_id, default_pending_state_days, invoice_id):
        _FIELDS_TO_READ = ['pending_state_id', 'change_date',
                           'invoice_id', 'days_to_next_state', 'end_date']
    ...
    def _set_pending_state_with_calculated_days(self, cursor, uid, new_pending_id, model_id, invoice_id, pending_state_days, context):
        model_obj = self.pool.get(context['model'])
        pending_history_obj = self.pool.get('account.invoice.pending.history')
