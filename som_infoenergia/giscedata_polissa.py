...
class GiscedataPolissaInfoenergia(osv.osv):
        ...
        if 'emp_allow_recieve_mail_infoenergia' in vals:
            ...
            if vals['emp_allow_recieve_mail_infoenergia']:
                profile_id = profile_obj.search(
                    cursor, uid, [('name', '=', 'Default profile')])[0]
    ...
    def get_consum_anual_consum_lectures(self, cursor, uid, polissa_id, context=None):
        ...
        limit_date = (datetime.today() - timedelta(self.MINIM_DIES_CONSUM)
                      ).strftime('%Y-%m-%d')
        ...
        if not lect_ids or lect_info[0]['name'] > limit_date or \
                lect_info[-1]['name'] < limit_date:
            return False

        consum = sum([x['consum'] for x in lect_info])
        n_dies = datetime.strptime(
            lect_info[-1]['name'], '%Y-%m-%d') - datetime.strptime(lect_info[0]['name'], '%Y-%m-%d')
        n_dies = n_dies.days
    ...
    def get_consum_anual_factures(self, cursor, uid, polissa_id, context=None):
        ...
        (datetime.today() - timedelta(self.MINIM_DIES_CONSUM)).strftime('%Y-%m-%d')
        from_date = (datetime.today() - relativedelta(months=14)).strftime('%Y-%m-%d')
        ...
        fact_infos = fact_obj.read(cursor, uid, fact_ids, [
                                   'energia_kwh', 'type', 'data_inici', 'data_final'])
        fact_infos.sort(key=lambda x: x['data_inici'])
        n_dies = datetime.strptime(fact_infos[-1]['data_final'], '%Y-%m-%d') - \
