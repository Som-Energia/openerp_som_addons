...
class SomInfoenergiaEnviament(osv.osv):
    ...
    def _attach_pdf(self, cursor, uid, ids, filepath):
        ...
        attachment_to_delete = attachment_obj.search(
            cursor, uid, [("res_id", "=", ids), ("res_model", "=", "som.infoenergia.enviament")]
        )
        ...
        with open(filepath, "r") as pdf_file:
            ...
            values = {
                ...
                "datas_fname": strip_accents(
                    u"{}_{}.pdf".format(enviament.polissa_id.name, enviament.lot_enviament.name)
                ),
    ...
    def render_header_data(self, cursor, uid, env_id, pdf_filepath, output_dir, context=None):
        ...
        if pol_data["persona_fisica"] == "NI":
            ...
            dades_linia_enviament.update(
                {"name": name_surnames["nom"], "surname": " ".join(name_surnames["cognoms"])}
            )
        else:
    ...
    def send_single_report(self, cursor, uid, _id, context=None):
        ...
        attachment_id = attach_obj.search(
            cursor, uid, [("res_id", "=", _id), ("res_model", "=", "som.infoenergia.enviament")]
        )
        if attachment_id:
        ...
        vals = {"from": tmpl.enforce_from_account.id, "attachment_ids": [(6, 0, [attachment_id])]}
