...
class SomInfoenergiaEnviament(osv.osv):
        ...
        with open(filepath, "r") as pdf_file:
            ...
            values = {
                "name": "Lot {}, informe {}, contracte {}".format(
                    ...
                    enviament.lot_enviament.tipus_informe.upper(),
                    enviament.polissa_id.name,
                ),
                "datas_fname": strip_accents(
                    u"{}_{}.pdf".format(
                        enviament.polissa_id.name, enviament.lot_enviament.name
                    )
                ),
    ...
    def render_header_data(
        self, cursor, uid, env_id, pdf_filepath, output_dir, context=None
    ):
        base_dir = os.path.dirname(os.path.abspath(__file__))
        template_name = os.path.join(
            base_dir, os.path.join("report", "infoenergia_header.mako")
        )
        ...
        dades_linia_enviament.update(
            {
                ...
                "lang": partner_obj.read(cursor, uid, pol_data["titular"][0], ["lang"])[
                    "lang"
                ],
            }
        )
    ...
    def send_reports(self, cursor, uid, ids, context=None):
        ...
        create_jobs_group(
            cursor.dbname,
            uid,
            _("Enviament Infoenergia Lot {} - {} enviaments").format(lot_name, len(ids)),
        ...
        if not amax_proc:
            amax_proc = None
        aw = AutoWorker(
            queue="infoenergia_send", default_result_ttl=24 * 3600, max_procs=amax_proc
        )
        aw.work()
    ...
    def send_single_report(self, cursor, uid, _id, context=None):
        ...
        if not polissa.emp_allow_recieve_mail_infoenergia:
            message = (
                u"La pòlissa no té habilitada la opció de rebre correus d'Infoenergia"
            )
    ...
    def poweremail_create_callback(self, cursor, uid, ids, vals, context=None):
        ...
        for _id in ids:
            self.write(
                ...
                uid,
                _id,
                {"estat": "encuat", "mail_id": origin_ids.get(_id, False)},
            )
        ...
        for item in ids:
            ...
            if "polissa_id" not in pol or not pol["polissa_id"]:
                ...
                auto_type = gp_obj.read(
                    cursor, uid, pol["polissa_id"][0], ["autoconsumo"]
                )["autoconsumo"]
    ...
    _columns = {
        ...
        "name": fields.related(
            "polissa_id", "name", type="char", string=_("Num Polissa"), readonly=True
        ),
        "autoconsum": fields.function(
            _ff_te_autoconsum,
            store=True,
            type="boolean",
            string=_("Té Autoproducció"),
            readonly=True,
            method=True,
        ),
        "tarifa": fields.related(
            "polissa_id", "tarifa", "name", type="char", string=_("Tarifa"), readonly=True
        ),
        ...
        "tipus_informe_lot": fields.related(
            "lot_enviament",
            "tipus_informe",
            type="char",
        ...
        "found_in_search": fields.boolean(
            _("La pòlissa relacionada estava present a alguna cerca")
        ),
