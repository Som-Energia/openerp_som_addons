...
class power_prices():
    ...
    def get_data(self, cursor, uid, object, extra_text, context):
        ...
        if polissa and polissa.tarifa.codi_ocsum in ('020', '021', '022', '023', '025', '019', '024'):
            ...
            if data_final:
                data_llista_preus = min(datetime.strptime(
                    data_final, '%Y-%m-%d'), datetime.today())
                ctx['date'] = data_llista_preus

            periodes_potencia = sorted(
                polissa.tarifa.get_periodes('tp', context=ctx).keys())

            for p in periodes_potencia:
                preus['preu_{0}'.format(p)] = get_atr_price(
                    cursor, uid, polissa, p, 'tp', ctx, with_taxes=False)[0]
