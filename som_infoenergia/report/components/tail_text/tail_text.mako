...
<p>${_(u"La contractació amb tarifa indexada està supeditada al pagament d’una garantia en forma de dipòsit bancari executable en cas d’impagament.")}</p>
<p>${_(u"Aquest dipòsit de garantia està calculat a partir de l’ús d’energia fet servir l’últim any, aplicant la nostra predicció de preu indexat de l'electricitat dels mesos vinents (no preu fix com fins ara), sense els descomptes provisionals que actualment està aplicant l’Estat als impostos i sense el descompte en els càrrecs que es van aplicar fins al desembre. Així, l’import del dipòsit és %s." % d.import_garantia)}</p>
...
<p>${_(u"El pagament s’haurà de fer efectiu al compte bancari de Som Energia dins dels 7 dies posteriors a la signatura del contracte i en tot cas, abans del %s" % d.data_limit_ingres)}</p>