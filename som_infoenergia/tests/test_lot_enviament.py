...
class LotEnviamentTests(testing.OOTestCase):
    ...
    def test_create_single_enviament_updateEnviament(self):
        ...
        lot_enviament.create_single_enviament_from_object(
            pol_id, context={'tipus': 'infoenergia'})
        ...
        self.assertEqual(env_obj.read(cursor, uid, updated_env, ['body_text'])[
                         0]['body_text'], 'Updated Text')
    ...
    def test_create_single_enviament_polissaInactive(self):
        ...
        post_enviaments = env_obj.search(cursor, uid,
                                         [("lot_enviament", "=", lot_enviament_id),
                                          ('estat', "=", 'baixa'), ('polissa_id', '=',
                                                                    pol_id), ('found_in_search', '=', False),
                                          ('info', 'ilike', 'La pòlissa {} està donada de baixa'.format(
                                              pol_name))
                                          ])
    ...
    def test_create_single_enviament_from_object(self):
        ...
        post_enviaments = env_obj.search(cursor, uid,
                                         [("lot_enviament", "=", lot_enviament_id),
                                          ('estat', "=", 'preesborrany'), ('polissa_id',
                                                                           '=', pol_id), ('found_in_search', '=', True),
    ...
    def test_cancel_enviaments_from_polissa_names_altres(self, mock_add_info):
        ...
        mock_add_info.assert_called_with(
            self.cursor, self.uid, mock.ANY, 'Enviament cancel·lat per Test cancel from polissa list')
