...
class WizardSendReportsTests(testing.OOTestCase):
    ...
    @mock.patch(
        "som_infoenergia.som_infoenergia_enviament.SomInfoenergiaEnviament.send_reports"
    )
    def test_send_reports_from_lot(self, mocked_send_reports):
        ...
        env_ids = env_obj.search(
            cursor,
            uid,
            [("lot_enviament", "=", lot_enviament_id), ("estat", "=", "obert")],
        )
...
class WizardCancelFromCSVTestsAndAddContractsLot(testing.OOTestCase):
    ...
    def test_wizard_invalid_active_id__invalid_cateogry(
        ...
        wiz_id = wiz_obj.create(
            cursor, uid, {"category": "InexistantCategory"}, context=context
        )
        wiz_obj.add_contracts_lot(cursor, uid, [wiz_id], context)
    ...
    def test_cancel_from_csv__one(self, mock_cancel):
        ...
        mock_cancel.assert_called_with(
            self.cursor, self.uid, lot_enviament_id, ["0001"], ctx
        )
        self.assertTrue(
...
class WizardCreateEnviamentsFromPartner(testing.OOTestCase):
    ...
    def test_wizard_create_enviaments_from_partner_esSocia(self, mock_add_partners):
        ...
        soci_ids = soci_obj.search(
            self.cursor, self.uid, [("data_baixa_soci", "=", False)]
        )
        partner_ids = [
            x["partner_id"][0]
    ...
    def test_wizard_create_enviaments_from_partner_teaportacions(self, mock_add_partners):
        ...
        gen_ids = gen_obj.search(
            self.cursor, self.uid, [("emission_id.type", "=", "apo")]
        )
        member_ids = [
            x["member_id"][0]
            for x in gen_obj.read(self.cursor, self.uid, gen_ids, ["member_id"])
        ]
        soci_obj = self.openerp.pool.get("somenergia.soci")
        partner_ids = [
            x["partner_id"][0]
