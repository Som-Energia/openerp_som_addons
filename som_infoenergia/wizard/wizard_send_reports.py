...
class WizardSendReports(osv.osv_memory):
    ...
    def send_reports(self, cursor, uid, ids, context=None):
        ...
        elif context.get("from_model") in [
    ...
    _columns = {
        "state": fields.selection(
            STATES, _(u"Estat del wizard d'enviament del lot de reports")
        ),
        "is_test": fields.boolean("Test", help=_(u"És un lot de Test?")),
        "is_from_lot": fields.boolean(
            "Cridat des de lot", help=_(u"Accionat des de lot?")
        ),
        "email_to": fields.char(
            u"E-mail on enviar els correus",
            size=256,
            help=u"Tots els enviaments s'enviaran a aquesta adreça",
        ),
        "email_subject": fields.char(
            "Assumpte correu",
            size=200,
            help="Assumpte que substituirà el de la plantilla",
        ),
        "allow_reenviar": fields.boolean(
            "Reenviar correus ja enviats",
            help=_(u"Permetre que es reenviïn correus ja enviats?"),
        ),
        "n_max_mails": fields.integer(
            "Màxim de correus (0 per enviar-los tots)",
            help=_(u"Número màxim de correus que es volen enviar. 0 per enviar-los tots"),
        ),
    }
