...
class SomInfoenergiaLotEnviament(osv.osv):
    ...
    def create_enviaments_from_object_list(
        self, cursor, uid, ids, object_ids, context=None
    ):
        if isinstance(ids, (tuple, list)):
            ids = ids[0]
        ...
        if not amax_proc:
            amax_proc = None
        aw = AutoWorker(
            queue="infoenergia_create_enviament",
            default_result_ttl=24 * 3600,
            max_procs=amax_proc,
        )
        aw.work()
    ...
    def create_single_enviament_from_object_async(
        self, cursor, uid, ids, object_id, context=None
    ):
        self.create_single_enviament_from_object(cursor, uid, ids, object_id, context)

    def create_single_enviament_from_object(
        self, cursor, uid, ids, object_id, context=None
    ):
        if isinstance(ids, (tuple, list)):
            ids = ids[0]
        ...
        env_id = env_obj.search(
            cursor,
            uid,
            [("lot_enviament", "=", ids), (context["from_model"], "=", object_id)],
        )
        if not env_id:
        ...
        if not pol_ids:
            pol_inactive_ids = (
                pol_obj.search(
                    cursor,
                    uid,
                    [("name", "=", env_data["contractid"]), ("active", "=", False)],
                )
                if env_data["contractid"] != "True"
                else None
            )
        ...
        if field_name == "total_enviaments":
            for _id in ids:
                ...
                res[_id] = env_obj.search_count(
                    cursor, uid, [("lot_enviament.id", "=", _id)]
                )
            ...
            for _id in ids:
                ...
                res[_id] = env_obj.search_count(
                    cursor,
                    uid,
                    [("lot_enviament.id", "=", _id), ("found_in_search", "=", True)],
                )
    ...
    _columns = {
        ...
        "total_esborrany": fields.function(
            _ff_totals,
            string="Enviaments en esborrany",
            readonly=True,
            type="integer",
            method=True,
        ),
        ...
        "total_baixa": fields.function(
            ...
            type="integer",
            method=True,
            help="Enviaments que tenen associat una pòlissa de baixa o amb data de baixa",  # noqa: E501
        ),
        "total_errors": fields.function(
            _ff_totals,
            string="Enviaments amb error",
            readonly=True,
            type="integer",
            method=True,
        ),
