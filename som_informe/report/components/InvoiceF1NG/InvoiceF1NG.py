...
class InvoiceF1NG:
    ...
    def get_data(self, cursor, uid, wiz, invoice, context={}):
        ...
        result["numero_edm"] = (
            f1.importacio_lectures_ids[0].comptador
            if f1 and f1.importacio_lectures_ids
            else ""
        )
        ...
        if f1:
            if f1.tipo_factura_f1 == "atr":
                for linia in f1.importacio_lectures_ids:
                    ...
                    dict_linia["magnitud_desc"] = get_description(
                        linia.magnitud, "TABLA_43"
                    )
                    dict_linia["periode_desc"] = get_description(
                        linia.periode, "TABLA_42"
                    )
                    dict_linia["lectura_inicial"] = linia.lectura_desde
                    dict_linia["lectura_final"] = linia.lectura_actual
                    dict_linia["consum_entre"] = (
                        linia.lectura_actual - linia.lectura_desde
                    )
