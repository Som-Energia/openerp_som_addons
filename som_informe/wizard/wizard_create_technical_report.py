...
class WizardCreateTechnicalReport(osv.osv_memory):
    ...
    _columns = {
        ...
        "lang": fields.many2one(
            ...
            required=True,
            select=1,
            help="Llengua de l'informe tècnic",
        ),
    ...
    def get_folder_data(self, cursor, uid, wiz):
        ...
        folder_hash = erp_config.get(
            cursor,
            uid,
            folder_data[subfolder]["config_id"],
            folder_data[subfolder]["config_value"],
        )
    ...
    def generate_report(self, cursor, uid, ids, context=None):
        ...
        document_binary = report_printer.create(
            cursor, uid, [wiz.id], data, context=context
        )
    ...
    def get_data(self, cursor, uid, id, context=None):
        ...
        if seleccionats:
            ...
            result_crono.extend(
                self.extract_switching_metadata(cursor, uid, sw_ids, context)
            )
    ...
    def get_columns(self, cursor, uid):
        ...
        no_function_fields = [
            ...
            if not isinstance(
                pol_obj._columns[field],
                (fields.function, fields.related, fields.property),
            )
        ]
        function_fields = [
            field
