...
class WizardUpdateInvoiceOVCheckBox(osv.osv_memory):
    ...
    def _default_info(self, cursor, uid, context=None):
        ...
        return _(
            u'S\'actualitzara el camp per {} factures'.format(len(fact_ids))
        )
    ...
    def update_check_ov_box(self, cursor, uid, ids, context=None):
        ...
        fact_obj.write(
            cursor, uid, fact_ids, {'visible_ov': action}, context=context
        )

        self.write(
            cursor, uid, ids,
            {'info': _(
                u'S\'han actualitzar correctament les {} factures'.format(
                    len(fact_ids))),
                'state': 'end'
             }, context=context)
    ...
    _columns = {
        'action': fields.selection(
            [('marcar', 'Visible'), ('desmarcar', 'No visible')], 'Accio'),
