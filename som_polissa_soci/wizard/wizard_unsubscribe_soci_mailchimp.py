...
class WizardUnsubscribeSociMailchimp(osv.osv_memory):
    ...
    def unsubscribe_soci_address(self, cursor, uid, ids, context=None):
        ...
        info_wizard = (
            "S'ha iniciat el procés de desubscripció de les adreces següents: \n"
        )
        ...
        for address in address_ids:
            ...
            is_member = soci_obj.search(
                ...
                uid,
                [
                    ("partner_id", "=", address_data["partner_id"][0]),
                    ("baixa", "=", False),
                ],
            )
            if is_member:
        ...
        self.write(
            cursor, uid, ids, {"info": info_wizard, "state": "end"}, context=context
        )
    ...
    _defaults = {
        "info": _(
            u'Aquest assitent desubscriu el Soci de la llista de Mailchimp de "Socis"'
        ),
        "state": lambda *a: "init",
    }
