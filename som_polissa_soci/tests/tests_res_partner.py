...
class SomenergiaSociTests(testing.OOTestCase):
    ...
    @mock.patch(
        "som_polissa_soci.res_partner_address.ResPartnerAddress.archieve_mail_in_list"
    )
    @mock.patch(
        "som_polissa_soci.res_partner_address.ResPartnerAddress.get_mailchimp_list_id"
    )
    ...
    def test__arxiva_client_mailchimp__withAddress(
        ...
        address_list = self.ResPartner.read(
            self.cursor, self.uid, partner_id, ["address"]
        )["address"]
