...
class TestsPartnerAddress(testing.OOTestCase):
    def setUp(self):
        ...
        self.cursor, self.uid, self.pool = (
            self.txn.cursor,
            self.txn.user,
            self.openerp.pool,
        )
    ...
    def test_fill_merge_fields_clients(self):
        ...
        partner_id = partner_address_o.read(
            self.cursor, self.uid, address_id, ["partner_id"]
        )["partner_id"][0]
        ...
        partner_address_o.write(
            self.cursor, self.uid, address_id, {"id_municipi": municipi_id}
        )
    ...
    def test_update_client_email_in_all_lists(self, mock_fakemailchimp, mock_md5):
        ...
        mock_fakemailchimp.update_list_member.assert_any_call(
            1, subscriber_hash, client_data
        )
        mock_fakemailchimp.update_list_member.assert_any_call(
            2, subscriber_hash, client_data
        )

    @mock.patch.object(
        res_partner_address.ResPartnerAddress, "archieve_mail_in_list_sync"
    )
    @mock.patch("som_polissa_soci.tests.tests_partner_address.fake_mchimp_client.lists")
