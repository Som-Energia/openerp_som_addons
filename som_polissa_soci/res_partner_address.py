...
class ResPartnerAddress(osv.osv):
    ...
    def get_mailchimp_list_id(list_name, mailchip_conn):
        all_lists = mailchip_conn.lists.get_all_lists(
    ...
    def fill_merge_fields_soci(self, cursor, uid, id, context=None):
        ...
        mailchimp_member = {
            ...
            'merge_fields': {
                ...
                FIELDS_SOCIS['Telefon']: partner_data['phone'] if partner_data['phone'] else partner_data['mobile'],
            }}
    ...
    @job(queue="mailchimp_tasks")
    def subscribe_mail_in_list(self, cursor, uid, clients_data, list_id, mailchimp_conn, context=None):
    ...
    def update_client_email_in_all_lists(self, cursor, uid, ids, old_email, email, mailchimp_conn, context=None):
        ...
        for _id in ids:
            for mchimp_list in all_lists:
                ...
                client_data["merge_fields"] = {
                    FIELDS_NO_MEMBERS['email']: email
                }

                try:
                    mailchimp_conn.lists.update_list_member(
                    ...
                    mailchimp_conn.lists.create_list_member_note(
                        list_id, new_subscriber_hash, {
                            "note": "S'ha actualitzat l'e-mail {} per {} vía ERP".format(old_email, email)
                        }
                    )
