...
socis = [s["partner_id"] and s["partner_id"][0] for s in O.SomenergiaSoci.read([], ["partner_id"])]

search_params = [("category_id", "=", soci_category)]
if socis:
    search_params += [("id", "not in", socis)]

partner2soci_ids = O.ResPartner.search(search_params, 0, 0, "id", {"active_test": False})
