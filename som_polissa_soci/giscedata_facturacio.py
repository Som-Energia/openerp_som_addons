...
class GiscedataFacturacioFacturador(osv.osv):
    ...
    def fact_via_lectures(self, cursor, uid, polissa_id, lot_id, context=None):
        ...
        donatiu_pol = polissa_obj.read(cursor, uid, polissa_id, ["donatiu"], ctx)[
            "donatiu"
        ]
        if donatiu_pol:
            ...
            factures_creades_modificables = [
                ...
                if f["data_inici"] >= start_date_bo_social
        ...
        if factures_creades_modificables:
            ...
            journal_codes = self.pool.get("res.config").get(
                cursor,
                uid,
                "som_invoice_bo_social_journal_codes",
                ["ENERGIA", "ENERGIA.R"],
            )
...
class GiscedataFacturacioFactura(osv.osv):
    ...
    _columns = {
        "visible_ov": fields.boolean(
            "Visible OV",
            help=_(
                u"Camp per indicar si la factura " u"sera visible a la Oficina Virtual"
            ),
        )
    }
