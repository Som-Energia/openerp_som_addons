...
class ResPartnerTest(testing.OOTestCase):
    ...
    def related_contracts(self, partner_id, relations):
        return self.ResPartner.related_contracts(
            self.cursor, self.uid, partner_id, relations
        )
    ...
    def name_and_cups(self, contract_id):
        contract = self.GiscedataPolissa.read(
            self.cursor, self.uid, contract_id, ["name", "cups"]
        )
        return dict(
            name=contract["name"],
    ...
    def test_modified_partners__previouslyUnrelated(self):
        self.GiscedataPolissa.write(
            self.cursor, self.uid, self.contract1, dict(titular=False)
        )
        result = self.contract_modified_partners(
            self.contract1,
            titular=self.owner1,
    ...
    def test_modified_partners__duplicatedRemoved(self):
        ...
        self.assertItemsEqual(
            result, [self.notified1, self.owner1, self.owner2, self.payer2]
        )
