...
class WizardGisceReGenerarOfertaSom(osv.osv_memory):
    ...
    def send_email(self, cursor, uid, info, errors, energia, context=None):
        ...
        self.pool.get('poweremail.templates').generate_mail(cursor, uid, template_id, [1], context=ctx)
    ...
    def _cron_giscere_daily_offer(self, cursor, uid, context=None):
        ...
        auto_generate_omie = literal_eval(conf_obj.get(cursor, uid, 'representacio_auto_generar_oferta_omie', '0'))
        auto_publish_omie = literal_eval(conf_obj.get(cursor, uid, 'representacio_auto_publicar_oferta_omie', '0'))
        auto_mail_notify = literal_eval(conf_obj.get(cursor, uid, 'representacio_auto_omie_mail_notification', '0'))
        ...
        for d in range(n_days):
            ...
            for unitat in unitats:
                ...
                try:
                    ...
                    wiz_id = self.create(cursor, uid, {'data_oferta': data_oferta, 'unitat_oferta': unitat})
                    wiz_oferta = self.browse(cursor, uid, wiz_id, context=context)
                    oferta_id = wiz_oferta.action_generar()
                    if oferta_id:
                        msg = "S'ha creat l'oferta diària amb ID {} per la Unitat d'Oferta {} pel día {}".format(
                        ...
                        if res:
                            msg = "S'ha marcat com a publicada l'oferta diària amb ID {} per la Unitat d'Oferta {} pel día {}".format(
                                oferta_id, unitat_oferta, data_oferta
                            )
                            logger.info(msg)
                            ...
                            if auto_generate_omie:
                                ...
                                oferta_data = oferta_obj.read(cursor, uid, oferta_id, ['oferta_a_omie', 'oferta_omie_id'])
                                if oferta_data.get('oferta_a_omie', 'no_generada') == 'generada':
                                    msg = "S'ha generat oferta d'OMIE per la Unitat d'Oferta {} pel día {}".format(
                                        oferta_id, unitat_oferta, data_oferta
                                    )
                                    logger.info(msg)
                                    ...
                                    if auto_publish_omie:
                                        ...
                                        omie_oferta_obj.enviar_oferta(cursor, uid, [omie_oferta_id], context=context)
                                        oferta_new_state = oferta_obj.read(cursor, uid, oferta_id, ['oferta_a_omie'])
                                        if oferta_new_state.get('oferta_a_omie', 'generada') == 'enviada':
                                            msg = "L'oferta d'OMIE per la Unitat d'Oferta {} pel día {} ha estat acceptada.".format(
                                                unitat_oferta, data_oferta
                                            )
                                            logger.info(msg)
                                            read_vals = ['texto_respuesta', 'id_unidad', 'total_energia']
                                            omie_data = omie_oferta_obj.read(cursor, uid, omie_oferta_id, read_vals)
                                            res_omie = omie_data['texto_respuesta'].split('\n')[0]
                                            info.append(res_omie)
                                            ener = '{}: {} MWh pel dia {}.'.format(
                                                omie_data['id_unidad'], omie_data['total_energia'], data_oferta
                                            )
                                            energia.append(ener)
                                        elif oferta_new_state.get('oferta_a_omie', 'generada') == 'enviada_error':
                                            msg = "L'oferta d'OMIE per la Unitat d'Oferta {} pel día {} ha retornat error.".format(
                                                unitat_oferta, data_oferta
                                            )
                                            logger.info(msg)
                                            errors.append(msg)
                                        else:
                                            msg = "L'oferta d'OMIE per la Unitat d'Oferta {} pel día {} no s'ha enviat.".format(
                                                unitat_oferta, data_oferta
                                            )
                                            logger.info(msg)
                                            errors.append(msg)
                                    ...
                                    msg = "No s'ha generat oferta d'OMIE per l'oferta diària amb ID {} per la Unitat d'Oferta {} pel día {}".format(
                                        oferta_id, unitat_oferta, data_oferta
                                    )
                                    logger.info(msg)
                                    errors.append(msg)
                            ...
                            msg = "No s'ha marcat com a publicada l'oferta diària amb ID {} per la Unitat d'Oferta {} pel día {}".format(
                                oferta_id, unitat_oferta, data_oferta
                            )
                            logger.info(msg)
                            errors.append(msg)
                        ...
                        error_msg = "No s'ha pogut crear ni publicar l'oferta diària de la Unitat d'Oferta {} pel día {}.".format(
                            unitat_oferta, data_oferta
                        )
