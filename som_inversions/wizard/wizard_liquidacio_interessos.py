...
class WizardLiquidacioInteressos(osv.osv_memory):
        ...
        vals.update(
            {
                ...
                "type": "in_invoice",
                "name": "%s%s%s"
                % (wiz.journal.code, wiz.date_end.split("-")[0], partner.ref),
    ...
    def calc(self, cursor, uid, ids, context=None):  # noqa: C901
        ...
        logger.notifyChannel(
            ...
            netsvc.LOG_INFO,
            _(
                u"Calculem liquidació aportacions de {0} comptes ({1},{2})."
                u" pre calc: {3}"
            ).format(
                total_acc, wiz.account.id, wiz.account_prefix, pre_calc and "Si" or "No"
            ),
        )
        counter = 0
        ...
        for account in account_obj.browse(cursor, uid, acc_ids):
            ...
            for mls in (
                ...
                partners.add(
                    aml_obj.read(cursor, uid, mls["id"], ["partner_id"])["partner_id"][0]
                )
            for partner in partners:
                ...
                if inv_obj.search_count(new_cursor, uid, [("name", "=", invoice_name)]):
                    msg[key]["err"] += [
                        ("  Ja existeix la factura %s creada." % invoice_name)
                    ]
                if not pre_calc:
                    if not mls[0].partner_id:
                        raise osv.except_osv(
                            _("Error"),
                            _(
                                u"El moviment %s de la compta %s no té empresa "
                                u"assignada"
                            )
                ...
                if not pre_calc:
                    ...
                    inv_obj.write(
                        new_cursor, uid, inv_id, {"check_total": inv.amount_total}
                    )
                    new_cursor.commit()
