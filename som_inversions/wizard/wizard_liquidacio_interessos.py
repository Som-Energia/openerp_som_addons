...
class WizardLiquidacioInteressos(osv.osv_memory):
    ...
    def create_invoice(self, cursor, uid, ids, partner_id, context=None):
        ...
        vals.update({
            ...
            'type': 'in_invoice',
            'name': '%s%s%s' % (wiz.journal.code, wiz.date_end.split('-')[0],
                                partner.ref),
    ...
    def create_invoice_line(self, cursor, uid, ids, vals, context=None):
        ...
        inv = self.pool.get('account.invoice').browse(cursor, uid,
                                                      vals['invoice_id'])
        l_vals = {}
    ...
    def calc(self, cursor, uid, ids, context=None):
        ...
        if not acc_ids:
            raise osv.except_osv(
                _(u'Error Compte'),
                _(u"No s'ha trobat cap compte per liquidar amb el prefix "
                  u"{0}").format(wiz.account_prefix))

        total_acc = len(acc_ids)
        logger.notifyChannel(
            'liquidacio APO', netsvc.LOG_INFO,
            _(u"Calculem liquidació aportacions de {0} comptes ({1},{2})."
              u" pre calc: {3}").format(
                total_acc, wiz.account.id, wiz.account_prefix,
                pre_calc and 'Si' or 'No'
            )
        )
        counter = 0
        #acc_ids = acc_ids[counter:]
        errors_factures_no_creades = []
        ...
        for account in account_obj.browse(cursor, uid, acc_ids):
            ...
            counter += 1
            if account_obj.read(cursor, uid, [account.id], ['balance'])[0]['balance'] >= 0 and not wiz.force:
            #if account.balance >= 0 and not wiz.force:
                continue
            key = '%s %s' % (account.code, account.name)
            logger.notifyChannel(
                'liquidacio APO', netsvc.LOG_INFO,
                _(u"{0:05d}/{1:05d} Compte {2}.").format(
                    counter, total_acc, key
                )
            )
...
#                                    order='date asc')
            ...
            for mls in aml_obj.q(new_cursor, uid).read(['id'], order_by=('id.asc',)).where(search_params):
                try:
                    ...
                    print("LINIA SENSE PARTNER: {}".format(str(mls)))
            for partner in partners:
                ...
                if not pre_calc:
                    ...
                    try:
                        ...
                        log_msg = "{}{}\n{}".format('error en crear factura pel partner: ', mls[0].partner_id, str(e))
                        errors_factures_no_creades.append(log_msg)
                        continue
                    inv_ids.append(inv_id)
                year_days = isleap(int(wiz.date_end.split('-')[0])) and 366 or 365
                for idx, ml in enumerate(mls):
                    if not ml.partner_id:
                        msg[key]['err'] += ['  Moviment: %s No Ref. Empresa '
                                            'assignada.' % ml.name]
            ...
            if not pre_calc:
                new_cursor.commit()
                new_cursor.close()
        print("************Factures no creades***********************")
        print(errors_factures_no_creades)
        if pre_calc:
            out = {'err': '', 'calc': ''}
            for key in msg:
                for t in ('err', 'calc'):
            ...
            logger.notifyChannel(
                'liquidacio APO', netsvc.LOG_INFO,
                _(u"Pre calc: err:{0} out:{1}).").format(
                    out['err'], out['calc']
                )
            )
            new_cursor.commit()
