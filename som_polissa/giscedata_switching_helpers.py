...
class GiscedataSwitchingHelpers(osv.osv):
    ...
    def move_bateria_virtual_from_old_to_new_post_hook(self, cursor, uid, old_pol_id, new_pol_id, context=None):
        if context is None:
            context = {}
        ...
        if polissa_obj.te_bateria_virtual(cursor, uid, old_pol_id, context=context):
            ...
            new_pol_vals = polissa_obj.read(cursor, uid, new_pol_id, ['data_alta', 'name'], context=context)
            for old_pol_bat in old_polissa.bateria_ids:
                # Canviem el nom de la bateria (BV -> FS)
                write_vals = {
                    'name': 'FS{}'.format(new_pol_vals['name'])
                }
