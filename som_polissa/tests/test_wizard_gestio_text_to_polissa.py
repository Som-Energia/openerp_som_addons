...
class TestWizardGestioTextToPolissa(testing.OOTestCaseWithCursor):
    ...
    def test_wizard_gestio_text_to_polissa__from_polissa(self):
        ...
        wiz_id = wiz_o.create(self.cursor, self.uid, wiz_init, context={'active_ids': [pol_id], 'from_model': 'giscedata.polissa'})

        wiz_o.modify_gestio_to_polissa(self.cursor, self.uid, [wiz_id], {'active_ids': [pol_id], 'from_model': 'giscedata.polissa'})
        new_comment = pol_o.read(self.cursor, self.uid, pol_id, ['info_gestions_massives'])['info_gestions_massives']
    ...
    def test_wizard_gestio_text_to_polissa__from_polissa__manyPolisses(self):
        ...
        wiz_init = {'comment':'comentari test 1', 'field_to_write': 'info_gestions_massives'}
        wiz_id = wiz_o.create(self.cursor, self.uid, wiz_init, context={'active_ids': pol_ids, 'from_model': 'giscedata.polissa'})

        wiz_o.modify_gestio_to_polissa(self.cursor, self.uid, [wiz_id], {'active_ids': pol_ids, 'from_model': 'giscedata.polissa'})
        new_comments = pol_o.read(self.cursor, self.uid, pol_ids, ['info_gestions_massives'])
        self.assertEqual([x['info_gestions_massives'] for x in new_comments], ['comentari test 1\n','comentari test 1\n'])
    ...
    def test_wizard_gestio_text_to_polissa__from_polissa_gest_endarrerida(self):
        ...
        wiz_o.modify_gestio_to_polissa(self.cursor, self.uid, [wiz_id], {'active_ids': [pol_id], 'from_model': 'giscedata.polissa'})
        new_comment = pol_o.read(self.cursor, self.uid, pol_id, ['info_gestio_endarrerida', 'info_gestions_massives'])
    ...
    def test_wizard_gestio_text_to_polissa__replace_polissa_gest_endarrerida(self):
        ...
        wiz_init = {'comment':'comentari test 1', 'field_to_write': 'info_gestio_endarrerida', 'option':'substituir'}
        wiz_id = wiz_o.create(self.cursor, self.uid, wiz_init, context={'active_ids': [pol_id], 'from_model': 'giscedata.polissa'})
    ...
    def test_wizard_gestio_text_to_polissa__delete_polissa_gest_endarrerida(self):
        ...
        wiz_init = {'comment':'', 'field_to_write': 'info_gestio_endarrerida', 'option':'eliminar'}
        wiz_id = wiz_o.create(self.cursor, self.uid, wiz_init, context={'active_ids': [pol_id], 'from_model': 'giscedata.polissa'})
