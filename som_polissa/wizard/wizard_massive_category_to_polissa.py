        ...
        for active_id in active_ids:
            ...
            if afegir:
                if category_id not in category_list:
                    pol_obj.write(
                        cursor, uid, active_id, {"category_id": [(4, category_id)]}
                    )
                    info += _(
                        u"Afegida categoria seleccionada a la pòlissa {}\n".format(
                            pol_name
                        )
                    )
                    modified += 1
                    ...
                    info += _(
                        u"Polissa {} ja té la categoria seleccionada\n".format(pol_name)
                    )
                    not_modified += 1
            else:
                if category_id in category_list:
                    pol_obj.write(
                        cursor, uid, active_id, {"category_id": [(3, category_id)]}
                    )
                    info += _(
                        u"Treta categoria seleccionada a la pòlissa {}\n".format(pol_name)
                    )
                    modified += 1
                else:
                    info += _(
