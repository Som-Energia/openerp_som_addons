...
class PaymentMandate(osv.osv):
    ...
    def _is_business(self, cursor, uid, mandate):
        vat = mandate.debtor_vat[2:] if mandate.debtor_vat.startswith(
            'ES') else mandate.debtor_vat
    ...
    def sepa_particulars_data(self, cursor, uid, ids):
        ...
        data.update(self._get_formatted_address(
            cursor, uid, mandate.creditor_id.partner_id.address[0]))
        ...
        data_firma = datetime.strptime(
            mandate.date, '%Y-%m-%d') if mandate.date else datetime.now()
