...
class ResPartner(osv.osv):
    ...
    def button_assign_acc_1714(self, cursor, uid, ids, context=None):
        ...
        for partner_id in res:
            self.write(
                cursor, uid, int(partner_id), {"property_account_titols": res[partner_id]}
            )
        return True
    ...
    def button_assign_acc_1635(self, cursor, uid, ids, context=None):
        ...
        if ag_id:
            for partner in self.browse(cursor, uid, ids):
                self.write(
                    cursor, uid, int(partner.id), {"property_account_gkwh": ag_id[0]}
                )
            return True
        else:
            return False
    ...
    def adopt_contracts_as_member(self, cursor, uid, partner_id, context=None):
        ...
        for contract_id in contract_ids:
            contract = contract_obj.read(
                cursor, uid, contract_id, ["pagador", "titular", "soci"]
            )
            if contract["soci"]:
