...
def migrate(cursor, installed_version):
    ...
    if len(soci_cat_ids) > 1:
        ...
        logger.notifyChannel(
            ...
            "Ja existeix Soci Category antiga. %s" % soci_cat_ids
        )
        imd_data = imd_obj._get_obj(
            cursor, uid, 'som_partner_account', 'res_partner_category_soci'
        )
        imd_id = imd_obj._get_id(
            cursor, uid, 'som_partner_account', 'res_partner_category_soci'
        )
        ...
        logger.notifyChannel('migration', netsvc.LOG_INFO,
                             "Esborrat Soci Category nou: %s" % soci_nou)
        ...
        logger.notifyChannel(
            'migration', netsvc.LOG_INFO,
            "Soci Category no repetit. "
            "Ja existia o creat correctament %s " % soci_cat_ids[0])

    logger.notifyChannel('migration', netsvc.LOG_INFO,
                         "Soci Category Creat correctament")
