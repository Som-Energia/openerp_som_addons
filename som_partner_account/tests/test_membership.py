...
class TestAccountAccountSom(testing.OOTestCase):
    ...
    def setUp(self):
        ...
        self.member_category = self.get_ref(
            'som_partner_account',
    ...
    def getCategories(self, partner_id):
        ...
        partner = Partner.read(self.cursor, self.uid, partner_id, [
            'category_id',
        ])
    ...
    def test__become_member__whenMemberDroppedOut(self):
        ...
        old_member_id = Partner.become_member(self.cursor, self.uid, self.partner_id)
        Member.write(self.cursor, self.uid, old_member_id, dict(
        ...
        self.assertNsEqual(ns(member), u"""
            data_baixa_soci: false
            baixa: false
            id: {id}
    ...
    def assertContractPeople(self, contract_id, member, owner, payer):
        ...
        contract = Contract.read(self.cursor, self.uid, contract_id, [
            'titular', 'soci', 'pagador',
        ])
        ...
        self.assertNsEqual(ns(contract), u"""\
            id: {contract_id}
            titular: {owner}
            pagador: {payer}
            soci: {member}
    ...
    def test__button_assign_soci_seq__createsMember(self):
        ...
        member_id = Member.search(self.cursor, self.uid, [
            ('partner_id', '=', self.partner_id),
        ])[0]
