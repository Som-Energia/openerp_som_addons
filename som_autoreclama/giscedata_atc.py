...
class GiscedataAtc(osv.osv):
    ...
    def create_ATC_R1_029_from_atc_via_wizard(self, cursor, uid, atc_id, context=None):
        ...
        subtr_id = subtr_obj.search(cursor, uid, [("name", "=", "029")], context=context)[
            0
        ]
    ...
    def create_general_atc_r1_case_via_wizard(self, cursor, uid, case_data, context=None):
        ...
        atcw_obj.create_atc_case_from_view(
            cursor, uid, [wiz_id], ctx
        )  # creates the ATC case
        ...
        if case_data.get("crear_cas_r1", False):
            ...
            r1atcw_obj = self.pool.get(
                open_r1_wiz["res_model"]
    ...
    _STORE_STATE = {
        "som.autoreclama.state.history.atc": (change_state, ["change_date"], 10)
    }
