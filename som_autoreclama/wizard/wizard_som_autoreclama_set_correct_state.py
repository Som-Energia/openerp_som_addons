...
class WizardSomAutoreclamaSetCorrectState(osv.osv_memory):
    _name = "wizard.som.autoreclama.set.correct.state"
    ...
    def assign_state(self, cursor, uid, ids, context=None):
        ...
        correct_state_id = ir_obj.get_object_reference(
            cursor, uid, "som_autoreclama", "correct_state_workflow_" + namespace
        )[1]
        s_obj = self.pool.get("som.autoreclama.state")
        next_state_id = s_obj.browse(cursor, uid, correct_state_id)
...
WizardSomAutoreclamaSetCorrectState()
