...
class WizardMassiveCreateR1029(osv.osv_memory):
    ...
    def action_create_r1029(self, cursor, uid, ids, context=None):
        ...
        disable_state_id = ir_obj.get_object_reference(
            cursor, uid, 'som_autoreclama', 'disabled_state_workflow_atc')[1]
        ...
        for case_id in reversed(case_ids):
            ...
            if atc_id:
                try:
                    h_obj.historize(cursor, uid, case_id,
                                    next_state_id.id, None, atc_id, context)
                    msg += _("Cas {} estat canviat manualment a '{}'\n").format(case_id,
                                                                                next_state_id.name)
                except Exception as e:
                    msg += _("Cas {} error al canviar manualment a '{}' : {}\n").format(case_id,
                                                                                        next_state_id.name, str(e))
