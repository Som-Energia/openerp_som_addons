...
class SomAutoreclamaState(osv.osv):
    ...
    def do_action(self, cursor, uid, state_id, atc_id, context=None):
        ...
        state_data = self.read(
            cursor, uid, state_id, ["name", "active", "generate_atc_parameters"]
        )
    ...
    def _ff_generate_atc_parameters(
    ...
    def _fi_generate_atc_parameters(
        ...
        try:
            ...
            return {
                "warning": {
                    "title": _(u"Atenció!"),
                    "message": _(
                        "Els parametres entrats no tenen un format " "correcte de JSON"
                    ),
                }
            }
    ...
    _columns = {
        ...
        "is_last": fields.boolean(
            string=_(u"Últim"), help=_(u"Indica si es l'utim estat")
        ),
        ...
        "workflow_id": fields.many2one(
            "som.autoreclama.state.workflow", _(u"Workflow"), required=True
        ),
