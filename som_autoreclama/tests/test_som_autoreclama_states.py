...
class SomAutoreclamaStatesTest(SomAutoreclamaBaseTests):
    ...
    @mock.patch.object(
        som_autoreclama_state_history.SomAutoreclamaStateHistoryAtc, "create"
    )
    @mock.patch.object(giscedata_atc.GiscedataAtc, "create")
    def _test_create_atc__state_correct_in_history(
        self, mock_create_atc, mock_create_history
    ):
    ...
    def test_create_atc__first_state_correct_in_history_indirectly_a(self):
        ...
        state_0_id = self.search_in(
            "som.autoreclama.state", [("name", "ilike", "correc")]
        )
    ...
    def test_historize__second_state_correct_in_history_indirectly(self):
        ...
        state_0_dt = "2022-01-02"
        state_1_id = self.search_in(
            "som.autoreclama.state", [("name", "ilike", "desact")]
        )
        ...
        history_obj.historize(
            self.cursor, self.uid, new_atc_id, state_1_id, state_1_dt, state_1_st
        )
    ...
    def test_historize__third_state_correct_in_history_indirectly(self):
        ...
        state_0_dt = "2022-01-02"
        state_1_id = self.search_in(
            "som.autoreclama.state", [("name", "ilike", "reclam")]
        )
        state_1_dt = "2022-02-15"
        state_1_st = 3
        state_2_id = self.search_in(
            "som.autoreclama.state", [("name", "ilike", "desact")]
        )
        ...
        history_obj.historize(
            self.cursor, self.uid, new_atc_id, state_2_id, state_2_dt, state_2_st
        )
...
class SomAutoreclamaCreationWizardTest(SomAutoreclamaBaseTests):
    def test_create_general_atc_r1_case_via_wizard__atr_wihtout_r1_type_a(self):
        ...
        self.assertEqual(
            atc.tancar_cac_al_finalitzar_r1, new_case_data["tanca_al_finalitzar_r1"]
        )
...
class SomAutoreclamaEzATC_Test(SomAutoreclamaBaseTests):
    def build_atc(
        ...
        subtipus_id = self.search_in(
            "giscedata.subtipus.reclamacio", [("name", "=", subtype)]
        )
        ...
        log_obj.write(
            self.cursor, self.uid, atc.log_ids[1].id, {"date": today_minus_str(log_days)}
        )
...
class SomAutoreclamaConditionsTest(SomAutoreclamaEzATC_Test):
    def test_fit_atc_condition__001_c_no(self):
        ...
        cond_id = ir_obj.get_object_reference(
            ...
            self.uid,
            "som_autoreclama",
            "conditions_001_correct_state_workflow_atc",
        )[1]
    ...
    def test_fit_atc_condition__some(self):
        ...
        for test_data in test_datas:
            ...
            cond_id = ir_obj.get_object_reference(
                self.cursor, self.uid, "som_autoreclama", test_data["cond"]
            )[1]
    ...
    def test_fit_atc_condition__all(self):
        ...
        for cond_id in cond_ids:
            ...
            # test more
            atc_id = self.build_atc(
                subtype=cond.subtype_id.name, log_days=cond.days * 2, r1=True
            )
            atc_data = atc_obj.get_autoreclama_data(self.cursor, self.uid, atc_id, {})
            ok = cond_obj.fit_atc_condition(self.cursor, self.uid, cond_id, atc_data, {})
            self.assertEqual(
...
class SomAutoreclamaUpdaterTest(SomAutoreclamaEzATC_Test):
    ...
    def test_get_atc_candidates_to_update__none(self):
        ...
        atc_ids.append(self.build_atc())
        state_d_id = self.search_in(
            "som.autoreclama.state", [("name", "ilike", "desact")]
        )
        history_obj = self.get_model("som.autoreclama.state.history.atc")
        history_obj.historize(
            self.cursor, self.uid, atc_ids[-1], state_d_id, today_str(), False
        )
...
class SomAutoreclamaDoActionTest(SomAutoreclamaEzATC_Test):
    def test_do_action__deactivated(self):
        ...
        state_id = ir_obj.get_object_reference(
            self.cursor, self.uid, "som_autoreclama", "disabled_state_workflow_atc"
        )[1]
    ...
    def test_do_action__no_action(self):
        ...
        self.assertEqual(
            result["message"], u"Estat {} sense acció --> Ok".format(state.name)
        )
        self.assertTrue("created_atc" not in result)
    ...
    def test_do_action__ok(self):
        ...
        ir_obj = self.get_model("ir.model.data")
        state_id = ir_obj.get_object_reference(
            self.cursor, self.uid, "som_autoreclama", "first_state_workflow_atc"
        )[1]
        state = state_obj.browse(self.cursor, self.uid, state_id)
        ...
        self.assertEqual(
            result["message"],
            u"Estat {} executat, nou atc creat amb id {}".format(
