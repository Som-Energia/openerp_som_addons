...
class SomAutoreclamaStateUpdater(osv.osv_memory):
    ...
    def update_atcs_if_possible(self, cursor, uid, ids, context=None):
        ...
        msg += _("Sumari\n")
        msg += _(
            "casos ATC que han canviat d'estat: .................. {}\n".format(
                len(updated)
            )
        )
        msg += _(
        ...
        msg += _(
            "casos ATC que no han pogut canviar per un error: .... {}\n".format(
                len(errors)
            )
        )
        msg += _("\n")
    ...
    def update_atc_if_possible(self, cursor, uid, atc_id, context=None):
        ...
        return False, _(
            u"No compleix cap condició activa, examinades {} condicions."
        ).format(len(cond_ids))
    ...
    def _cronjob_state_updater_mail_text(self, cursor, uid, data=None, context=None):
        ...
        emails_to = filter(
            lambda a: bool(a), map(str.strip, data.get("emails_to", "").split(","))
        )
        if emails_to:
