...
class PowersmsSMSbox(osv.osv):
    ...
    def powersms_callback(self, cursor, uid, ids, func, vals=None, context=None):
        ...
        for model in ids_cbk:
            ...
            try:
                if vals:
                    getattr(src, self.callbacks[func])(
                        cursor, uid, ids_cbk[model], vals, ctx
                    )
                else:
    ...
    def read(self, cursor, uid, ids, fields=None, context=None, load="_classic_read"):
        ...
        ret = []
        if valid_select:
            ret = super(PowersmsSMSbox, self).read(
                cursor, uid, valid_select, fields, context, load
            )
        ...
        for id in ids:
            try:
                ...
                logger.notifyChannel(
                    ...
                    netsvc.LOG_ERROR,
                    _(
                        "Sending of SMS %s failed. Probable Reason: Could not loginto server\nError: %s"  # noqa: E501
                    )
                    % (id, error),
                )
    ...
    def historise(self, cr, uid, ids, message="", context=None, error=False):
        ...
        for psms_id in ids:
            history = (
                self.read(cr, uid, psms_id, ["history"], context).get("history", "") or ""
            )
    ...
    def check_mobile(self, mobile_number):
        if not re.match(
            r"((?:\+34)*|(?:0034)*)6[0-9]{8}|((?:\+34)*|(?:0034)*)7[0-9]{8}",
            mobile_number,
        ):
            return False
        return True
    ...
    _columns = {
        "psms_account_id": fields.many2one(
            "powersms.core_accounts", "User account", required=True
        ),
