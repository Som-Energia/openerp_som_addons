...
class PowersmsSendWizard(osv.osv_memory):
        ...
        if template.enforce_from_account:
            return [
                (
                    ...
                    "%s (%s)"
                    % (
                        template.enforce_from_account.name,
                        template.enforce_from_account.tel_id,
                    ),
                )
            ]
        else:
    ...
    def _get_template_to_value(self, cr, uid, field, context=None):
        ...
        if len(context["src_rec_ids"]) > 1:
            return ", ".join(
                [
                    self.get_value(
                        cr, uid, template, getattr(template, field), context, id
                    )
    ...
    def save_to_smsbox(self, cr, uid, ids, context=None):
        ...
        def get_end_value(id, value):
            ...
            ):  # Multiple Mail: Gets value from the template
            ...
            for number in psms_to_list:
                ...
                if template.lang:
                    ctx["lang"] = self.get_value(
                        cr, uid, template, template.lang, context, id
                    )
    ...
    _defaults = {
        ...
        "to": lambda self, cr, uid, ctx: self.pool.get(
            "powersms.core_accounts"
        ).filter_send_sms(cr, uid, self._get_template_to_value(cr, uid, "def_to", ctx)),
        "from": lambda self, cr, uid, ctx: self._get_template_value(
            cr, uid, "def_from", ctx
        ),
