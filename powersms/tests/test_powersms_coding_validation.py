    ...
    def test_migration_post_0001_set_lleidanet_provider_for_existing_acc(self):
        ...
        if six.PY2:
            ...
            with Transaction().start(self.database) as txn:
                ...
                cursor.execute("ALTER TABLE powersms_core_accounts DROP COLUMN provider_id")
                cursor.execute("DROP TABLE powersms_provider")
                mod.up(cursor, True)
                provider_obj = pool.get("powersms.provider")
                providers = provider_obj.search(cursor, uid, [], context={"active_test": False})
                self.assertTrue(providers)
                for p_id in providers:
                    self.assertTrue(
                        hasattr(
                            provider_obj, provider_obj._get_provider_function(cursor, uid, p_id)
                        )
                    )
                ...
                provider_on_account = acc_obj.read(cursor, uid, account_id, ["provider_id"])[
                    "provider_id"
                ]
