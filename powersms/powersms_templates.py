...
class PowersmsTemplates(osv.osv):
    ...
    def _get_model_name(self, cursor, uid, template_ids, field_name, arg, context=None):
        ...
        for template_id in template_ids:
            model_id = pwm_templ_obj.read(cursor, uid, template_id, ["object_name"])[
                "object_name"
            ]
            if not model_id:
    ...
    def get_value(self, cursor, user, recid, message=None, template=None, context=None):
        ...
        if message:
            try:
                ...
                object = self.pool.get(template.object_name.model).browse(
                    cursor, user, recid, ctx
                )
                env = context.copy()
                env.update(
                    {
    ...
    _columns = {
        ...
        "def_from": fields.char(
            "Remitent (From)", size=250, help="Public name of remitent"
        ),
        "def_to": fields.char(
            "Recepients (To)",
            size=250,
