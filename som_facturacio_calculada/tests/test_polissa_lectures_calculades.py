...
class PolissaLecturesCalculadesTest(testing.OOTestCase):
    ...
    @mock.patch.object(
        giscedata_polissa.GiscedataPolissaCalculada,
        "_check_conditions_polissa_calculades",
    )
    def test_crear_lectures_calculades__sense_lectures(
        ...
        mock_check_conditions_polissa_calculades.side_effect = (
            check_conditions_polissa_calculades
        )
        ...
        self.assertEqual(
            result[1],
            u"La pòlissa 0001C no compleix les condicions per que no té categoria",
        )
