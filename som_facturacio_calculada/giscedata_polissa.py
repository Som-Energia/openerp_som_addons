...
class GiscedataPolissaCalculada(osv.osv):
    ...
    def _check_conditions_polissa_calculades(self, cursor, uid, id, context):
        ...
        if (
            gp_pobresa_id
            and pol.category_id
            and gp_pobresa_id in [x.id for x in pol.category_id]
        ):
    ...
    def _check_conditions_lectures_calculades(self, cursor, uid, _id, context):
        ...
        mtr_ids = mtr_o.search(
            cursor, uid, [("polissa.id", "=", _id), ("active", "=", True)]
        )
        if not mtr_ids:
        ...
        if pol.modcontractual_activa.data_inici > pol.data_ultima_lectura_f1:
            return False, _(
                u" data d'inici de la modcon activa més gran que data de últim F1"
            )
    ...
    def crear_lectura_calculada(self, cursor, uid, _id, context=None):
        ...
        if not crear_lectures:
            return _(
                u"La pòlissa {} no compleix les condicions perquè {}".format(
                    pol_name, text
                )
            )
        ...
        for _date in [data_seguent_lect_21, data_seguent_lect_14, data_seguent_lect_7]:
            ...
            if lect_created:
                ...
                return _(
                    u"La polissa {} té lectures creades en data {}".format(
                        pol_name, _date
                    )
                )
        ...
        return _(
            u"La pòlissa {} no pot generar lectures per falta de corba.".format(pol_name)
        )
