...
class GiscedataPolissaCalculada(osv.osv):
    ...
    def crear_lectures_calculades(self, cursor, uid, ids, context=None):
        ...
        for _id in ids:
            ...
            if not mtr_ids:
                print "No s'ha trobat comptador actiu per la pòlissa ID: {}".format(_id)
                break
            if len(mtr_ids) != 1:
                print "Multiples comptadors actius per la pòlissa ID: {}".format(_id)
                break
            mtr_id = mtr_ids[0]
            ...
            if not data_ultima_lect:
                print "Pòlissa ID sense data ultima lectura: {}".format(_id)
                break
            ...
            if not tg_ids:
                print "Pòlissa ID amb data última {} sense corbes en la data {}: {}".format(data_ultima_lect, data_seguent_lect, _id)
                break
            vals = {
