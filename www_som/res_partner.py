...
class ResPartner(osv.osv):
    ...
    def _www_email_inv(self, cursor, uid, ids, name, value, fnct_inv_arg, context=None):
        ...
        for partner_id in ids:
            search = [("partner_id.id", "=", partner_id)]
            for addr in addr_obj.search(
                cursor, uid, search, context={"active_test": False}
            ):
                vals = {"email": value}
    ...
    def _www_provincia_inv(
        self, cursor, uid, ids, name, value, fnct_inv_arg, context=None
    ):
        if isinstance(value, bool) and not value:
            value = None
        if not isinstance(ids, (list, tuple)):
