...
set -e
unset TEST_MODE TEST_READY TEST_RELEASE
[[ "$code" == 143 ]] || fail "TERM exit code became $code"
assert_link "$shared/addon_one" "$case_root/original"
new_case alteration
...
cat > "$case_root/bin/ln" <<'FAIL_LN'
#!/usr/bin/env bash
[[ "$*" != *"$TEST_INSTALL_TARGET"* ]] || { printf 'forced ln failure\n' >&2; exit 73; }
exec /bin/ln "$@"
FAIL_LN
chmod +x "$case_root/bin/ln"
export TEST_INSTALL_TARGET="$worktree/addon_one"
set +e
PATH="$case_root/bin:$PATH" run_wrapper --no-requirements 2> "$case_root/error"
code=$?
set -e
unset TEST_INSTALL_TARGET
[[ "$code" != 0 ]] || fail "installation failure returned success"
assert_link "$shared/addon_one" "$case_root/original"
[[ ! -e "$state/manifest" && ! -e "$state/owner" ]] || fail "installation cleanup failed"
printf 'PASS: wrapper; restore_failure_exit=%s install_failure=restored\n' "$code"
