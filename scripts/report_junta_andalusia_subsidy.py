...
# Read CSV
with open('entrada_4.csv', mode='r') as file:
...
# Connect to ERP
c = Client(**ooop2erppeek(configdb.erppeek))
...
for data_dict in data_dict_list:
    pol_ids = pol_obj.search(
        [('titular_nif', 'like', '%' + data_dict['nif'])],
        context={'active_test': False},
    )
    print("NIF {}: {} contracts".format(data_dict['nif'], len(pol_ids)))
    ...
    for pol in polisses:
        ...
        print("    NIF {}: POL {}: {} factures".format(data_dict['nif'], pol.name, len(fact_ids)))
        factures = fact_obj.browse(fact_ids)
        for fact in factures:
            fact_line = OrderedDict()
            fact_line['tipo_registro_2'] = '2'
            fact_line['nif'] = data_dict['nif']
            fact_line['nombre'] = data_dict['nombre']
            fact_line['contrato'] = pol.name
            fact_line['tipo_via'] = ''
            fact_line['direccio'] = pol.cups_direccio
            ...
            fact_line['consumo_facturado'] = fact.energia_kwh
            ...
            fact_line['nif_sol'] = data_dict['nif']
            fact_line['nombre_sol'] = data_dict['nombre']
            fact_line['codigo_error'] = ''
            fact_line['motivo_error'] = ''
            output.append(fact_line)

with open('output_4_fix.csv', 'w') as csv_file:
