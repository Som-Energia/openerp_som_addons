...
class SomCrawlersTask(osv.osv):
    ...
    def _get_distri_name(self, cursor, uid, task_ids, field_name, arg, context=None):
        ...
        for task_id in task_ids:
            config_id = self.read(cursor, uid, task_id, ["configuracio_id"])[
                "configuracio_id"
            ]
            if not config_id:
                res[task_id] = False
    ...
    def _get_last_result(self, cursor, uid, task_ids, field_name, arg, context=None):
        ...
        for task_id in task_ids:
            ...
            if not run_ids:
                res[task_id] = False
                continue
            resultat_bool = result_obj.read(cursor, uid, run_ids[0], [field_name])[
                field_name
            ]
    ...
    _columns = {
        ...
        "task_step_ids": fields.one2many(
            "som.crawlers.task.step", "task_id", string=_(u"Passos")
        ),
    ...
    def executar_tasca(self, cursor, uid, id, context=None):
        ...
        classresult.write(
            ...
            uid,
            result_id,
            {"resultat_bool": resultat_correcte, "resultat_text": output},
        )
    ...
    def schedule_next_execution(self, cursor, uid, id, context=None):
        ...
        data_anterior = datetime.strptime(
            self.read(cursor, uid, id, ["data_proxima_execucio"])[
                "data_proxima_execucio"
            ],
            "%Y-%m-%d %H:%M:%S",
        )
