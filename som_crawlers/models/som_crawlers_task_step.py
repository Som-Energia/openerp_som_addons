...
class SomCrawlersTaskStep(osv.osv):
    ...
    def attach_files_screenshot(
        ...
        if os.path.exists(path_to_screenshot):
            for file_name in os.listdir(path_to_screenshot):
                self.attach_file(
                    cursor, uid, path_to_screenshot, file_name, result_id, context
                )
    ...
    def get_clean_attachment(self, cursor, uid, id, attachment_id):
        ...
        # Guardem att a disc
        working_path = self.get_output_path(
            cursor, uid
        ) + "/som_crawlers_import_{}".format(
    ...
    def upload_files(self, cursor, uid, id, result_id, context=None):
        ...
        if "nom_fitxer" in task_step_params:
            ...
            if os.path.exists(script_path):
                ...
                ret_value = os.system(
                    "{} {} {}".format(path_python, script_path, args_str)
                )
                if ret_value != 0:
    ...
    def download_ftp_files(self, cursor, uid, id, result_id, context=None):
        try:
            ...
            if "dir_list" in task_step_params:
                ...
                temp_folder = (
                    server_data["name"] + "_" + datetime.now().strftime("%Y%m%d%H%M%S")
                )
                ...
                for remote_file_path in file_list:
                    if remote_file_path.endswith(".xml") or remote_file_path.endswith(
                        ".zip"
                    ):
                        ...
                        if (
                            not local_file
                            or ftp_reg.read(cursor, uid, local_file[0])["state"]
                            != "imported"
                        ):
                ...
                with zipfile.ZipFile(
                    destination_path + "/" + zip_filename, "w"
                ) as zipObj:
                    for filename in filenames:
