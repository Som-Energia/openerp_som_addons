...
class WizardFraccionarViaExtralines(osv.osv_memory):
    ...
    def action_fraccionar_via_extralines(self, cursor, uid, ids, context=None):
        ...
        data_final = (datetime.strptime(wiz.data_inici, "%Y-%m-%d")
        ...
        for info in factura_o.read(cursor, uid, factura_ids, ['number']):
            ...
            try:
                ...
                comment_head = '{} ({}): Fraccionament extralines en {} quotes.\n'.format(
                    ...
                    ''.join([word[0] for word in user_o.read(
                        cursor, uid, uid, ['name'])['name'].split(' ')]),
                    wiz.ntermes
                )
        ...
        if has_errors:
            msgs = [_(u"Hi ha hagut problemes al processar {0} factures: ").format(
                len(msgs))] + msgs
            msg = "\n\n".join(msgs)
        else:
