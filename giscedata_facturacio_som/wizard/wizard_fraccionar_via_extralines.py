...
class WizardFraccionarViaExtralines(osv.osv_memory):
    ...
    def action_fraccionar_via_extralines(self, cursor, uid, ids, context=None):
        ...
        if not wiz.first_term_payment:
            return super(
                WizardFraccionarViaExtralines, self
            ).action_fraccionar_via_extralines(cursor, uid, ids, context)
        ...
        for info in factura_o.read(cursor, uid, factura_ids, ["number"]):
            ...
            try:
                factura_teo = TransactionExecute(
                    cursor.dbname, uid, "giscedata.facturacio.factura"
                )
                ...
                comment_head = "{} ({}): Fraccionament extralines en {} quotes.\n".format(
                    ...
                    "".join(
                        [
                            word[0]
                            for word in user_o.read(cursor, uid, uid, ["name"])[
                                "name"
                            ].split(" ")
                        ]
                    ),
        ...
        msg = (
            _(u"Número de factures processades: {0}.\n\n").format(len(factura_ids)) + msg
        )
    ...
    _defaults = {
        ...
        "data_inici": lambda *a: (datetime.now() + timedelta(days=1)).strftime(
            "%Y-%m-%d"
        ),
    }
