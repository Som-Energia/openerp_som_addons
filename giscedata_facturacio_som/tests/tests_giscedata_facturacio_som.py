...
class TestGiscedataFacturacioSom(testing.OOTestCaseWithCursor):
    def setUp(self):
        ...
        gff_ids = self.gff_obj.search(
            self.cursor, self.uid, [("origin", "=", False)], limit=2
        )
        self.gff_obj.write(
            self.cursor, self.uid, gff_ids[0], {"origin": "sample_origin1"}
        )
        self.gff_obj.write(
            self.cursor, self.uid, gff_ids[1], {"origin": "sample_origin2"}
        )
    ...
    def test_search_exactExist_active(self):
        ...
        gff_ids = self.gff_obj.search(
            self.cursor, self.uid, [("number", "ilike", "0046/F")]
        )
