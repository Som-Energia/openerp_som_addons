...
class GiscedataAtc(osv.osv):
    ...
    def case_close_pre_hook(self, cursor, uid, ids, *args):
        ...
        treure = int(
            conf_obj.get(cursor, uid, "treure_facturacio_suspesa_on_cac_close", "0")
        )
        if treure:
    ...
    def case_cancel(self, cursor, uid, ids, *args):  # noqa: C901
        ...
        for atc_id in ids:
            ...
            if atc.state == "open" and atc.process_step == "03":
                raise osv.except_osv(
                    _(u"Warning"),
                    _(u"Cas ATC {} no es pot cancel·lar: R1 03 en estat Obert").format(
                        atc_id
                    ),
                )
            ...
            if atc.state == "pending" and atc.process_step == "04":
                raise osv.except_osv(
                    _(u"Warning"),
                    _(u"Cas ATC {} no es pot cancel·lar: R1 04 en estat Pendent").format(
                        atc_id
                    ),
                )
            ...
            if atc.process_step == "01" and r1 and r1.enviament_pendent:
