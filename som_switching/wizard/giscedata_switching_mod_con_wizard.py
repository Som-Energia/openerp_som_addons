...
from enerdata.contracts.tariff import (
    ...
    IncorrectMaxPower, NotAscendingPowers, NotNormalizedPower
)
import vatnumber
import json
...
class GiscedataSwitchingModConWizard(osv.osv_memory):
    ...
    def genera_casos_atr(self, cursor, uid, ids, context=None):
        ...
        if wizard.proces == 'M1' and change_type == "owner" and wizard.generate_new_contract == "create":
            new_vals = {}
        ...
        if wizard.casos_generats and wizard.contract.data_ultima_lectura:
            dultima = datetime.strptime(
                wizard.contract.data_ultima_lectura, "%Y-%m-%d"
            )
    ...
    _columns = {
        'necessita_documentacio_tecnica': fields.boolean(
            string="Necessita Documentació Tecnica",
            type="boolean"
        )
    }
