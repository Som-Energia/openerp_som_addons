...
class WizardSubtypeR1(osv.osv_memory):
    ...
    def action_create_r1_case_from_dict(
        self, cursor, uid, polissa_id, dict_fields, context=None
    ):
        ...
        sw_code = sw_obj.read(cursor, uid, res[-1], ["codi_sollicitud"])[
            "codi_sollicitud"
        ]
        new_text = _(
            u"{0}, {1}, Reclamació {2}: SUBTIPUS {3}. Generat el pas R1-01\n\n"
        ).format(
    ...
    _columns = {
        ...
        "refacturacio_pendent": fields.boolean(
            "Marcar contracte amb refacturacio pendent"
        ),
    }
