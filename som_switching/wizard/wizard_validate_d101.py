...
class GiscedataSwitchingWizardValidateD101(osv.osv_memory):
    ...
    def _get_change_reason(self, cursor, uid, ids):
        ...
        self._change_reason = d101_obj.read(cursor, uid, id_pas, ["motiu_canvi"])[
            "motiu_canvi"
        ]
    ...
    def validate_d101_autoconsum(self, cursor, uid, ids, context=None):
        ...
        if not is_rejected:
            pol_id = sw_obj.read(cursor, uid, sw_id, ["polissa_ref_id"])[
                "polissa_ref_id"
            ][0]
            d102_obj = self.pool.get("giscedata.switching.d1.02")

            try:
                m1_id = self._create_case_m1_01_autoconsum(
                    cursor, uid, ids, pol_id, context
                )
            ...
            message = "Passos D1-02 d'acceptació i M1-01 creats per al CUPS {}".format(
                cups_name
            )
    ...
    def _create_step_d1_02_autoconsum(
        ...
        if is_rejected:
            ...
            rebuig_obj.write(
                ...
                {
                    "desc_rebuig": d102.rebuig_ids[-1].desc_rebuig
                    + ": {}".format(rejection_comment)
                },
            )
    ...
    def _create_case_m1_01_autoconsum(self, cursor, uid, ids, pol_id, context=None):
        ...
        if (
            res_tarpot["warning"]
            and res_tarpot["warning"]["title"] != u"Potència incorrecte"
        ):
        ...
        casos_generats = wiz_obj.read(cursor, uid, wiz_id, ["casos_generats"])[0][
            "casos_generats"
        ]
    ...
    _columns = {
        "is_rejected": fields.boolean(
            "Rebutjar",
            help="Si s'activa, el D1-02 serà de rebuig, si no serà d'acceptació",
        ),
        ...
        "solicitud_tensio": fields.selection(
            [("", "")] + TABLA_64, u"Tensió Sol.licitada"
        ),
        ...
        "tariff": fields.selection(
            sorted(TABLA_17, key=lambda t: t[1]), string="Tarifa ATR"
        ),
