...
class WizardReexportLogAttachment(osv.osv_memory):
    ...
    def reexport_files(self, cursor, uid, ids, context=None):
        ...
        context.update({"update_logs": True})
        failed_files = []
        log_vals = log_obj.read(
            cursor, uid, log_ids, ["pas", "proces", "request_code", "cups_text"]
        )
        generated_files = []
        for log in log_vals:
            ...
            if not sw_id:
                failed_files.append(
                    (
                        log["cups_text"],
                        (
                            "No s'ha trobat un cas amb aquest codi de ",
            ...
            pas_obj = self.pool.get(
                "giscedata.switching.{}.{}".format(log["proces"].lower(), log["pas"])
            )
            pas_id = pas_obj.search(cursor, uid, [("sw_id", "=", sw_id)])
            try:
                step_id = step_obj.search(
                    cursor, uid, [("name", "=", log["pas"]), ("proces_id", "=", proces_id[0])]
                )
