...
class GiscedataSwitchingWizardR101(osv.osv_memory):
    ...
    def action_create_atr_case(self, cursor, uid, ids, context=None):
        winfo = self.read(cursor, uid, ids, ['facturacio_suspesa', 'refacturacio_pendent',
                                             'invoice', 'doc_add', 'doc_ids'])[0]
        context.update({
            'r1_doc_add': {
                'doc_add': winfo['doc_add'],
                'doc_ids': winfo['doc_ids']
            }
        })
        ...
        if winfo['facturacio_suspesa']:
            pol_obj.write(cursor, uid, factinfo['polissa_id'][0], {
    ...
    _columns = {
        ...
        'doc_ids': fields.many2many(
            'giscedata.switching.document', 'sw_wiz_r1_doc_ref',
            'wiz_r1_id', 'document_id', string='Documents',
