...
class TestWizardValidateD101(TestSwitchingImport):
    ...
    def test__validate_d101__accept(self):
        with Transaction().start(self.database) as txn:
            uid = txn.user
            cursor = txn.cursor
            ...
            d102_obj = self.openerp.pool.get('giscedata.switching.d1.02')
            m101_obj = self.openerp.pool.get('giscedata.switching.m1.01')
            wiz_obj = self.openerp.pool.get('wizard.validate.d101')
            ...
            wiz_id = wiz_obj.create(cursor, uid, wiz_init)
            wiz = wiz_obj.browse(cursor, uid, wiz_id)
            ...
            self.assertTrue(d102_id)
            self.assertTrue(m1_id)
            ...
            self.assertEqual(m1.polissa_ref_id.id, d1.polissa_ref_id.id)
