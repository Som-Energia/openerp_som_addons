...
class TestMultipleCommentF1(testing.OOTestCase):
    ...
    def test__multiple_comment_F1__oneF1(self):
        ...
        wiz_init = {'comment': ''}
        wiz_id = wiz_o.create(self.cursor, self.uid, wiz_init, context={'active_ids': [
                              inv_id], 'from_model': 'giscedata.facturacio.importacio.linia'})
        wiz_o.write(self.cursor, self.uid, [wiz_id], {'comment': 'comentari test 1'})

        wiz_o.modify_text_F1(self.cursor, self.uid, [wiz_id], {'active_ids': [
                             inv_id], 'from_model': 'giscedata.facturacio.importacio.linia'})
    ...
    def test__multiple_comment_to_F1__manyF1(self):
        ...
        wiz_init = {'comment': ''}
        wiz_id = wiz_o.create(self.cursor, self.uid, wiz_init, context={'active_ids': [
                              inv_id_1, inv_id_2], 'from_model': 'giscedata.facturacio.importacio.linia'})
        wiz_o.write(self.cursor, self.uid, [wiz_id], {'comment': 'comentari test 1'})
