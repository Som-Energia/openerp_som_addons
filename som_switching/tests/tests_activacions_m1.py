...
class TestActivacioM1(TestSwitchingImport):
    ...
    def get_m1_01_ct(self, txn, contract_id, tipus, context=None):
        ...
        new_partner_vat = self.ResPartner.read(cursor, uid, new_partner_id, ["vat"])[
            "vat"
        ][2:]
        ...
        return self.Switching.browse(
            cursor, uid, m101.sw_id.id, {"browse_reference": True}
        )
    ...
    def test_ct_subrogacio_baixa_mailchimp_ok(self, mock_function):
        with Transaction().start(self.database) as txn:
            ...
            old_partner_id = self.Polissa.read(cursor, uid, contract_id, ["titular"])[
                "titular"
            ][0]
    ...
    @mock.patch("som_polissa_soci.res_partner.ResPartner.arxiva_client_mailchimp_async")
    def test_ct_subrogacio_baixa_mailchimp_error__more_than_one_contract(
        self, mock_function
    ):
        with Transaction().start(self.database) as txn:
            cursor = txn.cursor
            ...
            m1 = self.get_m1_02_ct(
                txn, contract_id, "S", {"polissa_xml_id": "polissa_tarifa_018"}
            )
            with PatchNewCursors():
                self.Switching.activa_cas_atr(cursor, uid, m1)
    ...
    def test_ct_traspas_baixa_mailchimp_ok(self, mock_lectures, mock_mailchimp_function):
        with Transaction().start(self.database) as txn:
            ...
            self.Polissa.write(
                cursor, uid, pol_ids, {"titular": False, "data_baixa": "2099-01-01"}
            )
    ...
    def test_ct_traspas_baixa_mailchimp_error__more_than_one_contract(
        ...
        with Transaction().start(self.database) as txn:
            ...
            mock_lectures.return_value = []
            contract_id = self.get_contract_id(txn, "polissa_tarifa_018")
            # actualitze 'data_baixa' per a que no falle el test per la restricció de
            # dates 'giscedata_polissa_modcontractual_date_coherence'
