...
class GiscedataSwitchingHelpers(osv.osv):
    ...
    def activar_polissa_from_m1_canvi_titular_subrogacio(
        self, cursor, uid, sw_id, context=None
    ):
        res = super(
    ...
    def tancar_reclamacio_cac(self, cursor, uid, sw_id, context=None):
        ...
        Es marca el CAC com a "Tancar al finalitzar CAC",
        Cosa que provoca que el propi cas CAC es passi a finalitzat i el R1 també.
        """
        if context is None:
            context = {}
        ...
        autom_r1_fact = pas.subtipus_id.name in ["009", "036"] and (
            ...
            or (
                sw.state == "open"
                and pas.resultat == "01"
                and not polissa.refacturacio_pendent
            )
        )
        ...
        if autom_r1_fact:
            subtipus_info_str = "{0}-{1}".format(
                pas.subtipus_id.type, pas.subtipus_id.name
            )
    ...
    def ct_baixa_mailchimp(self, cursor, uid, sw_id, context=None):
        ...
        if pas01_info["sollicitudadm"] not in ("S", "A") or pas01_info[
            "canvi_titular"
        ] not in ("S", "T"):
            info = _(
                u"[Baixa Mailchimp] Aquest cas (M1-%s) no és d'un canvi de titular."
            ) % (step_name)
            return (_(u"OK"), info)
            ...
            data_activacio = m102_obj.read(
                cursor, uid, pas_actual.id, ["data_activacio"]
            )["data_activacio"]
            ...
            old_titular_id = pol_obj.read(
                cursor,
                uid,
                sw.cups_polissa_id.id,
                ["titular"],
                context={"date": data_previa},
            )["titular"][0]
