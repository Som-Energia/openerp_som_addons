...
class GiscedataSwitchingAparell(osv.osv):
    ...
    def calcula_lloguer_comptador(
        ...
        uom_alq_ids = [
            p["res_id"] for p in irdata_obj.read(cursor, uid, uom_alq_datas, ["res_id"])
        ]
        ...
        alq_ids = [
            p["res_id"] for p in irdata_obj.read(cursor, uid, alq_datas, ["res_id"])
        ]

        alq_vals = product_obj.read(
            cursor, uid, alq_ids, ["list_price", "code", "uom_id"]
        )
        ...
        preus = dict(
            [
                (
                    a["code"],
                    {"preu": a["list_price"], "uom": uoms["ALQ/mes"], "id": a["id"]},
                )
                for a in alq_vals
            ]
        )
        ...
        pm_vals = pm_obj.read(
            cursor, uid, aparell_vals["pm_id"][0], ["mode_lectura", "tensio_pm"]
        )
        ...
        preu = uom_obj._compute_price(
            cursor, uid, uoms["ALQ/mes"], preu_mes, uoms["ALQ/dia"]
        )
            ...
            for method_name in method_names:
                ...
                try:
                    ...
                    if len(res) == 2:
                        ...
                        raise Exception(
                            "ERROR",
                            _(u"La activacio no ha retornat la informacio esperada"),
                        )
                    msg_h = init_str + msg
                    tmp_cursor.commit()
                except Exception, e:
            ...
            if sw_obs["proces_id"][1].lower() in ["a3", "b1", "m1", "c1", "c2"]:
                ...
                if pas_id:
                    result[sw_obs["id"]] = pas_obj.read(
                        cursor, uid, pas_id[0], ["data_accio"]
                    )["data_accio"]
