...
class GiscedataFacturacioContracteLot(osv.osv):
    ...
    def date_invoice(self, cr, uid, id, context=None):
        ...
        try:
            ...
            if fact_ids:
                date_invoice = fact_obj.read(cr, uid, fact_ids[0], ["date_invoice"])[
                    "date_invoice"
                ]
    ...
    def _get_fact_origen(self, cr, uid, id, context=None):
        ...
        if fact_ids:
            for fact_id in fact_ids:
                ...
                energy_reading_vs = (
                    energy_readings_o.q(cr, uid).read(energy_readings_f).where(dmn)
                )
