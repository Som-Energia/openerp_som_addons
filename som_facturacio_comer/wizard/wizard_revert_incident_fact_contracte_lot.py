...
class WizardRevertIncidentFactCLot(osv.osv_memory):
    ...
    def delete_lot_factures_lectures_async(self, cursor, uid, wiz_id, pol_id, lot_id, c_lot_id, context={}):
        self.delete_lot_factures_lectures(cursor, uid, wiz_id, pol_id, lot_id, c_lot_id, context)

    def delete_lot_factures_lectures(self, cursor, uid, wiz_id, pol_id, lot_id, c_lot_id, context={}):
        ...
        fact_ids = fact_obj.search(cursor, uid, [('lot_facturacio','=', lot_id), ('polissa_id','=', pol_id), ('state', '=', 'draft')])
        if fact_ids:
        ...
        if context.get('delete_lectures', False):
            ...
            data_ultima_lectura = pol_obj.read(cursor, uid, pol_id, ['data_ultima_lectura'])['data_ultima_lectura']
    ...
    def do_action(self, cursor, uid, ids, context=None):
        ...
        for _id in selected_ids:
            ...
            if not c_lot.polissa_id.data_ultima_lectura:
                fact_ids = fact_obj.search(cursor, uid,
                    [('polissa_id', '=', c_lot.polissa_id.id), ('type', '=', 'out_invoice'),
                     '|', ('lot_facturacio', '!=', c_lot.lot_id.id), ('lot_facturacio', '=', False),
                    ],limit=1)

                if fact_ids:
                    error_msg.append(
                        u"Pòlissa {} té factures però no té Data última lectura facturada Real. No s'hi actua\n".format(c_lot.polissa_id.id)
                    )
                    continue
            job = self.delete_lot_factures_lectures_async(cursor, uid, ids[0], c_lot.polissa_id.id, c_lot.lot_id.id, _id, context)
        ...
        if job_ids:
            ...
            create_jobs_group(
                cursor.dbname, uid,
                _('Eliminar factures i reobrir {0} pòlisses amb incidència (eliminar lectures: {1}).').format(len(job_ids), context['delete_lectures']),
                'facturacio_tasks.delete_invoices', job_ids
            )
            amax_proc = int(self.pool.get("res.config").get(cursor, uid, "facturacio_tasks_delete_invoices_tasks_max_procs", "0"))
            if not amax_proc:
                amax_proc = None
            try:
                aw = AutoWorker(queue="delete_invoices", default_result_ttl=24 * 3600, max_procs=amax_proc)
                aw.work()
            except ValueError:
                logger.notifyChannel("objects", netsvc.LOG_WARNING, "Número de procesos no vàlid ({}). No s'arrencarà "
                                                                    "cap autoworker".format(amax_proc))
        error_msg = '\n'.join(error_msg)
        self.write(cursor, uid, [wiz.id],
            {'state': 'end', 'info': u"Pòlisses on actuar: {}\n{}\n\n Errors:\n{}".format(len(job_ids), '-'*10, error_msg)}
        )
    ...
    _columns = {
        ...
        'delete_lectures': fields.boolean(_("Delete lectures.lectura and lectures.potencia from comptador")),
