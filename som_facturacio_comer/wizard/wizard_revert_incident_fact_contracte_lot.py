...
class WizardRevertIncidentFactCLot(osv.osv_memory):
    ...
    def delete_lot_factures_lectures_async(self, cursor, uid, wiz_id, pol_id, lot_id, c_lot_id, context={}):
        self.delete_lot_factures_lectures(
            cursor, uid, wiz_id, pol_id, lot_id, c_lot_id, context)
    ...
    def delete_lot_factures_lectures(self, cursor, uid, wiz_id, pol_id, lot_id, c_lot_id, context={}):
        ...
        c_lot_obj.wkf_obert(cursor, uid, [c_lot_id], context={
                            'validate': False, 'from_lot': True})
    ...
    def do_action(self, cursor, uid, ids, context=None):
            ...
            if not c_lot.polissa_id.data_ultima_lectura:
                fact_ids = fact_obj.search(cursor, uid,
                                           [('polissa_id', '=', c_lot.polissa_id.id), ('type', '=', 'out_invoice'),
                                            '|', ('lot_facturacio', '!=',
                                                  c_lot.lot_id.id), ('lot_facturacio', '=', False),
                                            ], limit=1)
        ...
        if job_ids:
            ...
            create_jobs_group(
                cursor.dbname, uid,
                _('Eliminar factures i reobrir {0} pòlisses amb incidència (eliminar lectures: {1}).').format(
                    len(job_ids), context['delete_lectures']),
            ...
            if not amax_proc:
                amax_proc = None
            try:
                aw = AutoWorker(queue="delete_invoices",
