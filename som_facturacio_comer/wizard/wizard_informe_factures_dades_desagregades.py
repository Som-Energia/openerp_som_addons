...
def get_contract_ids(self, contracts):
    ...
    search_params = [('tarifa.codi_ocsum', 'in', ['019', '020', '021', '022', '023'])] #only 3.0TD and 6.xTD
    if pol_ids:
        search_params.append(('id', 'in', pol_ids))
...
def find_invoices(self, pol_ids, from_date, to_date):
    ...
    for pol_id in pol_ids:
        ...
        subitem = OrderedDict([('Contracte', pol.name), ('Tarifa Comercialitzadora', pol.llista_preu.name if pol.llista_preu else ''), ('Indexada', 'Indexada' if pol.llista_preu and 'indexada' in pol.llista_preu.name.lower() else 'No'), ('Energia activa', 0), 
                    ('MAG', 0), ('Penalització reactiva', 0), ('Potència', 0), ('Excés potència', 0), ('Excedents', 0), ('Lloguer comptador', 0), ('IVA', 0), ('IGIC', 0), ('IESE', 0), ('Altres', 0), ('TOTAL', 0)])
    ...
    def action_crear_informe(self, cursor, uid, ids, context=None):
        ...
        informe_data = self.read(
            cursor, uid, informe_id, ['from_date', 'to_date', 'contracts'])
        to_date = datetime.strftime(informe_data['to_date'], '%Y-%m-%d')
        from_date = datetime.strftime(informe_data['from_date'], '%Y-%m-%d')
        contracts = list(set(map(str.strip,str(informe_data['contracts']).split(','))))

        pol_ids = self.get_contract_ids(contracts)

        items = find_invoices(contracts, from_date, to_date)
        ...
        output.close()
        output_file = create_csv(items, from_date, to_date)
    ...
    def set_to_date(self):
        ...
        self.to_date = initial_date - timedelta(days=day_initial_date)

    def set_from_date(self):
        self.from_date = self.to_date - relativedelta(months=12) + timedelta(days=1)

    _columns = {
        'state': fields.char('Estat', size=16, required=True),
        'start_date': field.date('Data inici càlcul', required=True),
