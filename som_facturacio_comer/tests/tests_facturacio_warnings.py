...
class TestsFacturesValidation(testing.OOTestCase):
    ...
    def validation_warnings(self, factura_id):
        ...
        warning_ids = vali_obj.validate_invoice(
            self.txn.cursor, self.txn.user, factura_id
        )
        warning_vals = warn_obj.read(
            self.txn.cursor, self.txn.user, warning_ids, ["name"]
        )
    ...
    def prepare_contract(self, pol_id, data_alta, data_ultima_lectura):
        ...
        for meter in contract.comptadors:
            for lectura in meter.lectures:
                lectura.unlink(context={})
    ...
    def create_measure(
        self, meter_id, date_measure, measure, origen_code, origen_comer_code
    ):
        ...
        origins_comer_ids = (
            loc_obj.search(
                self.txn.cursor, self.txn.user, [("codi", "=", origen_comer_code)]
            )
            if origen_code
            else []
        )
    ...
    def crear_modcon(self, polissa_id, teoric_max, ini, fi):
        ...
        polissa_obj.write(
            cursor, uid, polissa_id, {"teoric_maximum_consume_gc": teoric_max}
        )
    ...
    def test_check_origin_readings_by_contract_category__contract_with_wrong_category(
        self,
    ):
        pol_id = self.get_fixture("giscedata_polissa", "polissa_0001")
    ...
    def test_check_min_periods_and_teoric_maximum_consum__contract_with_wrong_category(
        self,
    ):
        pol_id = self.get_fixture("giscedata_polissa", "polissa_0001")
        self.create_category_to_contract_by_name(pol_id, "Petit Contracte")
    ...
    def test_check_gkwh_G_invoices__contract_without_generationkWh(self):
        ...
        lot_id = self.cnt_lot_obj.read(cursor, uid, cnt_lot_id, ["lot_id"])["lot_id"][0]
        ...
        lot_dates = self.lot_obj.read(cursor, uid, lot_id, ["data_inici", "data_final"])
