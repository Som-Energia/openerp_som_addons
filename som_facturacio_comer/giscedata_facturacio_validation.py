...
class GiscedataFacturacioValidationValidator(osv.osv):
    ...
    def check_origin_readings_by_contract_category(self, cursor, uid, fact, parameters):
        ...
        if pcat_ids and pcat_ids[0] in [x.id for x in fact.polissa_id.category_id]:
            ...
            not_alowed_origins_comer_ids = loc_obj.search(
                cursor,
                uid,
                [("codi", "in", eval(parameters["lectures_origen_comer_codes"]))],
            )

            data_inici = datetime.strptime(fact.data_inici, "%Y-%m-%d") - timedelta(
                days=1
            )
    ...
    def check_consume_by_percentage_and_category(self, cursor, uid, fact, parameters):
        ...
        if pcat_ids and pcat_ids[0] in [x.id for x in fact.polissa_id.category_id]:
            ...
            if parameters.get("min_amount", False) and abs(
                fact.amount_total
            ) <= parameters.get("min_amount", 0.0):
                return None
            ...
            if (
                not max_consume
                and teoric_maximum_consume_gc
                and teoric_maximum_consume_gc > 0
            ):
    ...
    def check_consume_by_percentage(self, cursor, uid, fact, parameters):
        ...
        ).check_consume_by_percentage(cursor, uid, fact, parameters)
    ...
    def check_consume_by_amount(self, cursor, uid, fact, parameters):
        ...
        return super(
            GiscedataFacturacioValidationValidator, self
        ).check_consume_by_amount(cursor, uid, fact, parameters)

    def check_gkwh_G_invoices(
        self, cursor, uid, clot, data_inici, data_fi, parametres={}
    ):
        ...
        origen_comer_f1g_id = imd_obj.get_object_reference(
            cursor, uid, "giscedata_facturacio_switching", "origen_comer_f1_g"
        )[1]
        ...
        for modcontractual in modcon_obj.browse(cursor, uid, mod_ids):
            ...
            c_actius = polissa.comptadors_actius(
                data_inici_periode_f, data_final_periode_f, order="data_alta asc"
            )
            ...
            for compt in compta_obj.browse(cursor, uid, c_actius):
                ...
                data_inici_periode_f2 = (
                    datetime.strptime(data_inici_periode_f, "%Y-%m-%d")
                    - timedelta(days=1)
                ).strftime("%Y-%m-%d")
                ctx = {
                ...
                for periode, lectura in lectures_activa.items():
                    if (
                        "origen_comer_id" in lectura["actual"]
                        and lectura["actual"]["origen_comer_id"][0] == origen_comer_f1g_id
                    ):
                        return {}
