...
class Tests_FacturacioFacturaReport_base(testing.OOTestCase):
    ...
    def get_fixture(self, model, reference):
        ...
        return imd_obj.get_object_reference(
            self.txn.cursor, self.txn.user, model, reference
        )[1]
...
class Tests_FacturacioFacturaReport_fill_and_find(Tests_FacturacioFacturaReport_base):
    ...
    def test__find_all_components_data__detect_some_of_them(self):
        ...
        self.assertEquals(
            methods["logo"].im_class.__name__, "giscedata.facturacio.factura.report"
        )
        ...
        for name, method in methods.items():
            self.assertEquals(
                method.im_class.__name__, "giscedata.facturacio.factura.report"
            )

    @mock.patch.object(
        giscedata_facturacio_report.GiscedataFacturacioFacturaReport, "cleanup_data"
    )
...
class Tests_FacturacioFacturaReport_logo_component(Tests_FacturacioFacturaReport_base):
    def test__som_report_comp_logo__no_soci(self):
        ...
        self.assertEquals(
            result, {"logo": "logo_som.png", "has_agreement_partner": False}
        )
...
class Tests_FacturacioFacturaReport_invoice_details_energy(
    ...
    def test__som_report_comp_invoice_details_energy__without_energy_lines(
        ...
        l_ids = self.linia_f_obj.search(
            self.cursor, self.uid, [("tipus", "=", "energia")]
        )
...
class Tests_FacturacioFacturaReport_partner_info(Tests_FacturacioFacturaReport_base):
    def test__som_report_comp_partner_info(self):
        ...
        self.payment_type_obj.write(
            self.cursor,
            self.uid,
            f.polissa_id.tipo_pago.id,
            {"code": "TRANSFERENCIA_SBC"},
        )
        self.factura_obj.write(
            self.cursor, self.uid, f_id, {"partner_bank": partner_bank_id}
        )
