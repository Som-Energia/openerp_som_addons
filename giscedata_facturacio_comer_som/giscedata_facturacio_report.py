...
class GiscedataFacturacioFacturaReport(osv.osv):
    ...
    def get_origen_lectura(self, lectura):
        ...
        sin_lectura_id = origen_obj.search(
            self.cursor, self.uid, [('codi', '=', '99')])[0]
        ...
        # Busquem la tarifa
        tarifa_id = tarifa_obj.search(self.cursor, self.uid, [
                                      ('name', '=', lectura.name[:-5])])
        if tarifa_id:
    ...
    def get_readings_data_fill(self, fact):
        ...
        for lectura in fact.lectures_energia_ids:
            ...
            if lectura.tipus == 'activa' and lectura.magnitud == 'AE':
                ...
                lectura_real = sorted([lectura_real for lectura_real in lectura.comptador_id.pool_lectures if lectura_real.tipus == "A" and "{} ({})".format(lectura_real.periode.tarifa.name, lectura_real.periode.product_id.name)
                                      == lectura.name and lectura_real.origen_id.id not in [7, 9, 22, 23] and datetime.strptime(lectura_real.name, '%Y-%m-%d') < datetime.strptime(lectura.data_actual, '%Y-%m-%d')], reverse=True, key=lambda l: l.name)
                lectures_real_a.setdefault(lectura.comptador, [])
                if len(lectura_real) > 0:
                    lectures_real_a[lectura.comptador].append(
                        (lectura.name[-3:-1], val(lectura_real[0].lectura), dateformat(lectura_real[0].name)))
                else:
                    lectura_real = sorted([lectura_real for lectura_real in lectura.comptador_id.lectures if lectura_real.tipus == "A" and "{} ({})".format(lectura_real.periode.tarifa.name, lectura_real.periode.product_id.name)
                                          == lectura.name and lectura_real.origen_id.id not in [7, 9, 22, 23] and datetime.strptime(lectura_real.name, '%Y-%m-%d') < datetime.strptime(lectura.data_actual, '%Y-%m-%d')], reverse=True, key=lambda l: l.name)
                    if len(lectura_real) > 0:
                        lectures_real_a[lectura.comptador].append(
                            (lectura.name[-3:-1], val(lectura_real[0].lectura), dateformat(lectura_real[0].name)))
                ...
                lectura_real = sorted([lectura_real for lectura_real in lectura.comptador_id.pool_lectures if lectura_real.tipus == "R" and "{} ({})".format(lectura_real.periode.tarifa.name, lectura_real.periode.product_id.name)
                                      == lectura.name and lectura_real.origen_id.id not in [7, 22, 23] and datetime.strptime(lectura_real.name, '%Y-%m-%d') < datetime.strptime(lectura.data_actual, '%Y-%m-%d')], reverse=True, key=lambda l: l.name)
                lectures_real_r.setdefault(lectura.comptador, [])
                if len(lectura_real) > 0:
                    lectures_real_r[lectura.comptador].append(
                        (lectura.name[-3:-1], val(lectura_real[0].lectura), dateformat(lectura_real[0].name)))
                else:
                    lectura_real = sorted([lectura_real for lectura_real in lectura.comptador_id.lectures if lectura_real.tipus == "R" and "{} ({})".format(lectura_real.periode.tarifa.name, lectura_real.periode.product_id.name)
                                          == lectura.name and lectura_real.origen_id.id not in [7, 22, 23] and datetime.strptime(lectura_real.name, '%Y-%m-%d') < datetime.strptime(lectura.data_actual, '%Y-%m-%d')], reverse=True, key=lambda l: l.name)
                    if len(lectura_real) > 0:
                        lectures_real_r[lectura.comptador].append(
                            (lectura.name[-3:-1], val(lectura_real[0].lectura), dateformat(lectura_real[0].name)))
    ...
    def get_distri_phone(self, polissa):
        ...
        try:
            if partner_id:
                pa_ids = pa_obj.search(self.cursor, self.uid, [
                                       ('partner_id', '=', partner_id)])
                return (pa_obj.read(self.cursor, self.uid, [pa_ids[0]], ['phone'])[0]['phone']
    ...
    def get_atr_price(self, fact, tarifa, linia):
        ...
        if tipus == 'potencia':
            ...
            model_ids = model_obj.search(self.cursor, self.uid,
                                         [('module', '=', 'giscedata_facturacio'),
                                          ('name', '=', 'product_cosfi')])
            product_id = model_obj.read(self.cursor, self.uid,
                                        model_ids[0], ['res_id'])['res_id']
        ...
        uom_id = uom_obj.search(self.cursor, self.uid, [
                                ('name', '=', uom_data['uom_name'])])[0]
    ...
    def get_total_excess_power_consumed(self, fact):
        ...
        exces_potencia = sorted(sorted([l for l in fact.linia_ids if l.tipus == 'exces_potencia'], key=attrgetter(
            'price_unit_multi'), reverse=True), key=attrgetter('name'))
        if exces_potencia:
    ...
    def get_tarifa_elect_atr(self, fact, pricelist_name='pricelist_tarifas_electricidad'):
        ...
        model_ids = model_obj.search(self.cursor, self.uid,
                                     [('module', '=', 'giscedata_facturacio'),
                                         ('name', '=', pricelist_name)])
        if model_ids:
            res = model_obj.read(self.cursor, self.uid,
                                 model_ids[0], ['res_id'])['res_id']
    ...
    def max_requested_powers(self, pol, fact):
        ...
        required_max_requested_powers = is_2XTD(
            pol) and fact.date_invoice >= first_date_qr_pot_max
    ...
    def aplica_ajust_topall_gas(self, cursor, uid, fra, context=None):
        ...
        data_inici_ajust_topall_gas = varconf_o.get(
            cursor, uid, 'start_date_mecanisme_ajust_gas')
        data_fi_ajust_topall_gas = varconf_o.get(
            cursor, uid, 'end_date_mecanisme_ajust_gas')
    ...
    def get_efecto_reductor_precio_mayorista_mecanismo_ajuste(self, cursor, uid, fra, context=None):
        ...
        dmn = [
            ...
            ('local_timestamp', '<=', (datetime.strptime(val(fra.data_final),
             '%Y-%m-%d') + timedelta(days=1)).strftime('%Y-%m-%d %H:%M:%S')),
        ]
    ...
    def get_component_gdo_data(self, fact, pol):
        ...
        swich_date = conf_obj.get(self.cursor, self.uid,
                                  'gdo_and_impact_yearly_switch_date', '2099-05-01')
        ...
        if fact.date_invoice < swich_date:
            ...
            gdo_som = conf_obj.get(self.cursor, self.uid,
                                   'component_gdo_data', example_data_2020)
            try:
    ...
    def get_component_contract_data_data(self, fact, pol):
        ...
        data = {
            ...
            # fact.te_autoconsum and fact.polissa_id.autoconsum_id and fact.polissa_id.autoconsum_id.collectiu
            'is_autoconsum_colectiu': te_autoconsum_collectiu(fact, pol),
    ...
    def get_component_readings_table_data(self, fact, pol):
        ...
        """
        (periodes_a, _, _, _, lectures_a, _, _, _, _, _, _, _,
         total_lectures_a, _, _, _, _, _, _) = self.get_readings_data(fact)

        dies_factura = (datetime.strptime(fact.data_final, '%Y-%m-%d')
                        - datetime.strptime(fact.data_inici, '%Y-%m-%d')).days + 1
    ...
    def is_visible_readings_g_table(self, fact, pol):
        (_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _,
         has_adjust_g, has_readings_g) = self.get_readings_data(fact)
    ...
    def get_component_readings_g_table_data(self, fact, pol):
        ...
        return a dictionary with all readings table component needed data
        """
        (_, _, _, periodes_g, _, _, _, lectures_g, _, _, _, _, _, _,
         _, total_lectures_g, _, _, _) = self.get_readings_data(fact)
        data = {
    ...
    def get_component_readings_text_data(self, fact, pol):
        (periodes_a, _, _, _, lectures_a, _, _, lectures_g, _, _, _, _, _, _, _, _,
         has_adjust_a, has_adjust_g, has_readings_g) = self.get_readings_data(fact)
        data = {
            'periodes_a': periodes_a,
    ...
    def get_component_energy_consumption_graphic_data(self, fact, pol):
        ...
        (periodes_a, _, _, _, _, _, _, _, _, _, _, _, _,
         _, _, _, _, _, _) = self.get_readings_data(fact)
        ...
        historic_dies = (datetime.strptime(data_fin, '%Y-%m-%d')
                         - datetime.strptime(data_ini, '%Y-%m-%d')).days
        historic_dies_or_1 = historic_dies or 1.0

        mes_any_inicial = (datetime.strptime(
            fact.data_inici, '%Y-%m-%d') - timedelta(days=365)).strftime("%Y/%m")
    ...
    def get_component_energy_consumption_graphic_td_data(self, fact, pol):
        ...
        show_mean_zipcode_consumption = pol.tipo_medida == '05' and mean_zipcode_consumption_dates[
            'start'] <= fact.data_inici < mean_zipcode_consumption_dates['end']
    ...
    def get_component_meters_data(self, fact, pol):
        ...
        """
        (periodes_a, _, _, _, lectures_a, _, _, _, lectures_real_a, _, _, _,
         total_lectures_a, _, _, _, _, _, _) = self.get_readings_data(fact)
        show_meter = any([lectures_a[comptador][0][7] != "real" for comptador in sorted(
            lectures_a) if len(lectures_real_a[comptador]) > 0])
        data = {
    ...
    def get_component_invoice_details_power_data(self, fact, pol):
        ...
        for l in sorted(sorted(fact.linies_potencia, key=attrgetter('data_desde')), key=attrgetter('name')):
            days_year = is_leap_year(datetime.strptime(
                l.data_desde, '%Y-%m-%d').year) and 366 or 365
            power_lines.append({
                'name': l.name,
                'quantity': l.quantity,
    ...
    def get_component_invoice_details_other_concepts_data(self, fact, pol):
        ...
        if fiscal_position:
            ...
            is_excempcio_IE_base = len(percentatges_exempcio_splitted) == 3 and len(
                percentatges_exempcio) == 2
    ...
    def get_component_invoice_details_comments_data(self, fact, pol):
        ...
        """
        invoice_comment = fact.comment if fact.journal_id.code.startswith(
            'RECUPERACION_IVA') and fact.comment else None
        data = {
    ...
    def get_component_invoice_details_tec271_data(self, fact, pol):
        ...
        text = sup_territorials_tec_271_comer_obj.get_info_text(
    ...
    def is_visible_reactive(self, fact):
        (_, periodes_r, _, _, _, _, _, _, _, _, _, _, _,
         _, _, _, _, _, _) = self.get_readings_data(fact)
    ...
    def get_component_reactive_readings_table_data(self, fact, pol):
        (_, periodes_r, _, _, _, lectures_r, _, _, _, lectures_real_r, _, _,
         _, total_lectures_r, _, _, _, _, _) = self.get_readings_data(fact)
        data = {
    ...
    def get_component_maximeter_readings_table_data(self, fact, pol):
        ...
        exces_potencia = sorted(sorted([l for l in fact.linia_ids if l.tipus == 'exces_potencia'], key=attrgetter(
            'price_unit_multi'), reverse=True), key=attrgetter('name'))
        periodes_m = sorted(
            list(set([lectura.name for lectura in fact.lectures_potencia_ids])))
    ...
    def get_component_invoice_summary_data(self, fact, pol):
        total_exces_consumida, has_exces_potencia = self.get_total_excess_power_consumed(
            fact)
        impostos = {}
        iese = 0
        for l in fact.tax_line:
    ...
    def get_component_invoice_summary_td_data(self, fact, pol):
        ...
        inductive = self.sub_component_invoice_details_td_inductive_data_visibility(
            fact, pol)
        capacitive = self.sub_component_invoice_details_td_capacitive_data_visibility(
            fact, pol)
        ...
        data['total_boe17_2021'] = self.get_sub_component_invoice_details_td_power_discount_BOE17_2021_data(fact, pol)[
            'total']
        data['total_boe17_2021'] += self.get_sub_component_invoice_details_td_energy_discount_BOE17_2021_data(fact, pol)[
            'total']
    ...
    def get_component_readings_6x_data(self, fact, pol):
        ...
        for lect in lectures:
            ...
            if comptador_anterior and comptador_actual != comptador_anterior:
                comptadors[(comptador_anterior, data_actual_pre,
                            data_ant_pre)] = llista_lect
                llista_lect = []
            if data_act:
    ...
    def get_component_invoice_details_td_data(self, fact, pol):
        ...
        power_discount_BOE17_2021 = self.get_sub_component_invoice_details_td_power_discount_BOE17_2021_data(
            fact, pol)
        energy_discount_BOE17_2021 = self.get_sub_component_invoice_details_td_energy_discount_BOE17_2021_data(
            fact, pol)
        data = {
    ...
    def get_sub_component_invoice_details_td(self, fact, pol, linies, lectures=None):
        ...
        if lectures != None:
            for comptador_name, lectures_comptador in lectures.items():
                for lectura in lectures_comptador:
                    if lectura[0] == u'P1':
                        data = str(datetime.strptime(
                            lectura[4], '%d/%m/%Y').date() + timedelta(days=1))
                        origin = _(u'estimada')
        ...
        for l in sorted_linies:
            ...
            days_per_year = is_leap_year(datetime.strptime(
                l.data_desde, '%Y-%m-%d').year) and 366 or 365
            ...
            lines_data[block]['origin'] = readings[l.data_desde] if l.data_desde in readings else _(
                u'sense lectura')
            lines_data[block]['data'] = data_desde
            lines_data[block]['days'] = (datetime.strptime(
                l.data_fins, '%Y-%m-%d') - datetime.strptime(l.data_desde, '%Y-%m-%d')).days + 1
            lines_data[block]['date_from'] = dateformat(l.data_desde)
            lines_data[block]['date_to_d'] = val(l.data_fins) if 'date_to_d' not in lines_data[block] or lines_data[block]['date_to_d'] < val(
                l.data_fins) else lines_data[block]['date_to_d']
    ...
    def get_sub_component_invoice_details_td_power_discount_BOE17_2021_data(self, fact, pol):
        ...
        discount_lines = [l for l in fact.linia_ids if l.tipus in 'altres'
        ...
        data['dies'] = int(discount_power_lines['P1']['days']
                           ) if 'P1' in discount_power_lines else 0
    ...
    def get_sub_component_invoice_details_td_power_tolls_data(self, fact, pol):
        ...
        tarifa_elect_atr = self.get_tarifa_elect_atr(
            fact, 'pricelist_tarifas_peajes_electricidad')
        days_year = 0
        for l in sorted(sorted(fact.linies_potencia, key=attrgetter('data_desde')), key=attrgetter('name')):
            days_year = is_leap_year(datetime.strptime(
                l.data_desde, '%Y-%m-%d').year) and 366 or 365
        ...
        data['showing_periods'] = self.get_matrix_show_periods(pol)
        data['dies'] = int(atr_linies_potencia['P1']['days']
                           ) if 'P1' in atr_linies_potencia else 0
        data['dies_any'] = days_year
        return data
    ...
    def get_sub_component_invoice_details_td_power_charges_data(self, fact, pol, discount):
        ...
        tarifa_elect_atr = self.get_tarifa_elect_atr(
            fact, 'pricelist_tarifas_cargos_electricidad')
        ...
        for k, v in charges_lines_potencia.items():
            ...
            charges_lines_potencia[k] = {
                # TODO switch back when lines comes well calculated
                'preu_cargos': v['price'],
                # TODO switch back when lines comes well calculated
                'atr_cargos': v['our_cargos'] - d,
                'days': v['multi'],
            }
        data = charges_lines_potencia
        data['showing_periods'] = self.get_matrix_show_periods(pol)
        data['dies'] = int(charges_lines_potencia['P1']['days']
                           ) if 'P1' in charges_lines_potencia else 0
        data['dies_any'] = days_year
    ...
    def get_sub_component_invoice_details_td_energy_data(self, fact, pol, discount):
        (a, a, a, a, lectures_a, a, a, a, a, a, a, a, a,
         a, a, a, a, a, a) = self.get_readings_data(fact)
        linies_energia = self.get_real_energy_lines(fact, pol)
    ...
    def get_sub_component_invoice_details_td_other_concepts_data(self, fact, pol):
        ...
        data['header_multi'] = len(data['bosocial_lines']
                                   + data['altres_lines'] + data['donatiu_lines'])
        data['last_row'] = 'donatiu' if len(data['donatiu_lines']) else 'altres' if len(
            data['altres_lines']) else 'bosocial'
    ...
    def get_sub_component_invoice_details_td_excess_power_maximeter(self, fact, pol):
        ...
        excess_lines_data = self.get_sub_component_invoice_details_td(
            fact, pol, excess_lines)
    ...
    def get_component_amount_destination_td_data(self, fact, pol):
        ...
        pie_charges = round(float(fact.total_cargos),
                            2) if "total_cargos" in fact else 0.0
        ...
        pie_energy = round(pie_total - pie_renting
                           - pie_taxes - pie_tolls - pie_charges, 2)
        data = {
    ...
    def get_component_energy_consumption_detail_td_data(self, fact, pol):
        ...
        for meter in pol.comptadors:
            data = self.get_sub_component_energy_consumption_detail_meter_td_data(
                fact, pol, meter)
            if data['is_visible']:
                meters.append(data)
    ...
    def get_sub_component_energy_consumption_detail_meter_td_data(self, fact, pol, meter):
        ...
        active = self.get_sub_component_energy_consumption_detail_td_active_data(
            fact, pol, meter)
        surplus = self.get_sub_component_energy_consumption_detail_td_surplus_data(
            fact, pol, meter)
    ...
    def adjust_readings_priority(self, adjust_readings_list):
        adjust_readings_list = [
            adjust for adjust in adjust_readings_list if adjust != False]
    ...
    def get_sub_component_energy_consumption_detail_td_surplus_data(self, fact, pol, meter):
        (a, a, a, a, a, a, a, lectures_g, a, a, a, a, a,
         a, a, a, a, a, a) = self.get_readings_data(fact)
        lectures = lectures_g
        data = {
            'showing_periods': self.get_matrix_show_periods(pol),
        ...
        if meter.name in lectures:
            data['is_visible'] = len(
                lectures[meter.name]) > 0 and te_autoconsum_amb_excedents(fact, pol)
            for reading in lectures[meter.name]:
                data[reading[0]] = {
                    'initial': reading[1],
                    'final': reading[2],
    ...
    def get_sub_component_energy_consumption_detail_td_inductive_data(self, fact, pol, meter):
        (a, a, a, a, a, lectures_r, a, a, a, a, a, a, a,
         a, a, a, a, a, a) = self.get_readings_data(fact)
        data = {
            'showing_periods': self.get_matrix_show_periods(pol),
            'is_visible': False,
    ...
    def get_sub_component_energy_consumption_detail_td_capacitive_data(self, fact, pol, meter):
        (a, a, a, a, a, a, lectures_c, a, a, a, a, a, a,
         a, a, a, a, a, a) = self.get_readings_data(fact)
        data = {
            'showing_periods': self.get_matrix_show_periods(pol),
            'is_visible': False,
    ...
    def get_component_simplified_enviromental_impact_data(self, fact, pol):
        ...
        seid_som = conf_obj.get(self.cursor, self.uid,
                                'som_environmental_impact_data', example_som_2020)
        try:
            data['som'] = eval_and_json(seid_som)
        except Exception:
            data['som'] = eval_and_json(example_som_2020)

        seid_mit = conf_obj.get(self.cursor, self.uid,
                                'mitjana_environmental_impact_data', example_mitjana_2020)
        try:
