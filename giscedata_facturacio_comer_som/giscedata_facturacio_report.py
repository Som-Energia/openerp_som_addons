...
def is_TD(pol):
    if dummy_td:
        return True
    return pol.tarifa.codi_ocsum in (
        "018",
        "019",
        "020",
        "021",
...
class GiscedataFacturacioFacturaReport(osv.osv):
    ...
    def find_all_components_data(self, context=None):
        ...
        if context and "allow_list" in context:
            fill_methods = {
                a: b for (a, b) in fill_methods.items() if a in context["allow_list"]
            }
    ...
    def get_origen_lectura(self, lectura):
        ...
        sin_lectura_id = origen_obj.search(self.cursor, self.uid, [("codi", "=", "99")])[
            0
        ]
        estimada_som_id = origen_comer_obj.search(
            self.cursor, self.uid, [("codi", "=", "ES")]
        )[0]
        ...
        tarifa_id = tarifa_obj.search(
            self.cursor, self.uid, [("name", "=", lectura.name[:-5])]
        )
        if tarifa_id:
            ...
            if lectura.tipus == "activa" and lectura.magnitud == "AE":
                ...
                if len(lectura_real) > 0:
                    ...
                    lectura_real = sorted(
                        [
                            lectura_real
                            for lectura_real in lectura.comptador_id.lectures  # noqa: F812, E501
                            if lectura_real.tipus == "A"
                            and "{} ({})".format(
                                lectura_real.periode.tarifa.name,
                                lectura_real.periode.product_id.name,
    ...
    def get_atr_price(self, fact, tarifa, linia):
        ...
        if tipus == "potencia":
            ...
            product_id = model_obj.read(self.cursor, self.uid, model_ids[0], ["res_id"])[
                "res_id"
            ]
        ...
        uom_id = uom_obj.search(
            self.cursor, self.uid, [("name", "=", uom_data["uom_name"])]
        )[0]
    ...
    def get_tarifa_elect_atr(self, fact, pricelist_name="pricelist_tarifas_electricidad"):
        ...
        if model_ids:
            res = model_obj.read(self.cursor, self.uid, model_ids[0], ["res_id"])[
                "res_id"
            ]
    ...
    def get_lines_in_extralines(self, fact, pol):
        ...
        extra_linia_datas = extra_obj.read(
            self.cursor, self.uid, extra_ids, ["factura_linia_ids"]
        )
    ...
    def max_requested_powers(self, pol, fact):
        ...
        required_max_requested_powers = (
            is_2XTD(pol) and fact.date_invoice >= first_date_qr_pot_max
        )
    ...
    # Duplicated code until V108 and molule giscedata_omie_comer can be installed,
    # then remove it
    ...
    def get_efecto_reductor_precio_mayorista_mecanismo_ajuste(
        self, cursor, uid, fra, context=None
    ):
        ...
        dmn = [
            ...
            (
                ...
                "<=",
                (
                    datetime.strptime(val(fra.data_final), "%Y-%m-%d") + timedelta(days=1)
    ...
    def get_component_gdo_data(self, fact, pol):
        ...
        if fact.date_invoice < swich_date:
            ...
            gdo_som = conf_obj.get(
                self.cursor, self.uid, "component_gdo_data", example_data_2020
            )
            try:
    ...
    def get_component_contract_data_data(self, fact, pol):
        """
        returns a dictionary with all the contract data needed for the contract_data
        component
        """
        periodes_a = sorted(
            list(
        ...
        data = {
            ...
            "power_invoicing_type": pol.facturacio_potencia == "max"
            or len(periodes_a) > 3,
    ...
    def get_component_energy_consumption_graphic_data(self, fact, pol):
        ...
        historic_dies = (
            datetime.strptime(data_fin, "%Y-%m-%d")
    ...
    def get_component_energy_consumption_graphic_td_data(self, fact, pol):
        ...
        (
            required_max_requested_powers,
            max_potencies_demandades,
        ) = self.max_requested_powers(pol, fact)
    ...
    def get_component_emergency_complaints_data(self, fact, pol):
        ...
        data = {
            ...
            "distri_phone": ".".join(
                [dphone[i: i + 3] for i in range(0, len(dphone), 3)]
            )
            if "." not in dphone
            else dphone,
            "has_agreement_partner": pol.soci.ref in agreementPartners.keys(),
            "agreement_partner_name": pol.soci.name,
            "is_energetica": pol.soci.ref in agreementPartners.keys()
            and pol.soci.ref == "S019753",
    ...
    def get_component_invoice_details_power_data(self, fact, pol):
        ...
        for l in sorted(  # noqa: E741
            sorted(fact.linies_potencia, key=attrgetter("data_desde")),
            key=attrgetter("name"),
        ):
            days_year = (
                is_leap_year(datetime.strptime(l.data_desde, "%Y-%m-%d").year)
                and 366
                or 365
            )
            power_lines.append(
            ...
            l_name = l.name[:2]
            if l_name not in atr_linies_potencia.keys():
    ...
    def get_component_invoice_details_energy_data(self, fact, pol):
        ...
        for l in sorted(  # noqa: E741
            ...
            l_count = Counter(
                {"quantity": l.quantity, "atrprice_subtotal": l.atrprice_subtotal}
            )
            l_name = l.name[:2]
            if l_name not in atr_linies_energia.keys():
        ...
        data = {
            ...
            "is_new_tariff_message_visible": fact.data_final
            > new_tariff_with_power_tolls_date,
        }
        return data
    ...
    def get_component_invoice_details_generation_data(self, fact, pol):
        ...
        generation_lines = []
        for l in sorted(  # noqa: E741
            sorted(fact.linies_generacio, key=attrgetter("data_desde")),
            key=attrgetter("name"),
        ):
    ...
    def get_component_invoice_details_reactive_data(self, fact, pol):
        ...
        reactive_lines = []
        for l in sorted(  # noqa: E741
            sorted(fact.linies_reactiva, key=attrgetter("data_desde")),
            key=attrgetter("name"),
        ):
    ...
    def get_component_invoice_details_other_concepts_data(self, fact, pol):  # noqa: C901
        def get_tax_type(l):  # noqa: E741
            if l.tax_id.description == "IESE_RD_17_2021":
                ...
                return "0.5euroMWh"
            elif l.tax_id.description and l.tax_id.description.startswith(
                "IESE_RD_17_2021_85"
            ):
                return "excempcio"
            return "5.11percent"
        ...
        for l in fact.linia_ids:  # noqa: E741
            ...
            if l.tipus in (
                "altres",
                "cobrament",
            ) and l.invoice_line_id.product_id.code not in (
                "DN01",
                "BS01",
                "DESC1721",
                "DESC1721ENE",
        ...
        if fiscal_position:
            ...
            is_excempcio_IE_base = (
                ...
                and len(percentatges_exempcio) == 2
            )
    ...
    def get_component_invoice_details_comments_data(self, fact, pol):
        """
        returns a dictionary with all the comments data needed
        for the invoice details comments component
        """
        invoice_comment = (
            fact.comment
    ...
    def get_component_invoice_details_tec271_data(self, fact, pol):
        """
        returns a dictionary with all the tec271 data needed for the invoice
        details tec271 component
        """
    ...
    def get_component_invoice_summary_data(self, fact, pol):
        ...
        data = {
            ...
            "has_autoconsum": te_autoconsum(fact, pol),
            "autoconsum_total_compensada": sum(
                [l.price_subtotal for l in fact.linies_generacio]  # noqa: E741
            ),
            "impostos": impostos,
            "iese": iese,
            "donatiu": donatiu,
    ...
    def get_component_invoice_summary_td_data(self, fact, pol):
        ...
        data[
            "total_boe17_2021"
        ] = self.get_sub_component_invoice_details_td_power_discount_BOE17_2021_data(
            fact, pol
        )[
            "total"
        ]
        data[
            "total_boe17_2021"
        ] += self.get_sub_component_invoice_details_td_energy_discount_BOE17_2021_data(
            fact, pol
        )[
            "total"
        ]
        ...
        data["total_bosocial2023"] = sum(
            [l.price_subtotal for l in bosocial2023_lines]  # noqa: E741
        )
        return data
    ...
    def get_component_readings_6x_data(self, fact, pol):
        ...
        for lect in lectures:
            ...
            if comptador_anterior and comptador_actual != comptador_anterior:
                comptadors[
                    (comptador_anterior, data_actual_pre, data_ant_pre)  # noqa: F821
                ] = llista_lect
                llista_lect = []
            if data_act:
    ...
    def get_component_hourly_curve_data(self, fact, pol):
        ...
        data = {
            ...
            "has_agreement_partner": pol.soci.ref in agreementPartners.keys(),
            "factura_data": fact.get_curve_as_csv().get(fact.id, None)
            if is_visible
            else None,
        }
        data["profiled_curve"] = (
            data["factura_data"] and "REEPROFILE" in data["factura_data"]
        )
        return data
    ...
    def get_component_rectificative_banner_data(self, fact, pol):
        data = {
            "is_visible": bool(
                fact.journal_id.code == "RECUPERACION_IVA.R" and fact.rectifying_id
            )
        }
        if data["is_visible"]:
    ...
    def get_component_invoice_details_td_data(self, fact, pol):
        ...
        data = {
            ...
            "power_tolls": self.get_sub_component_invoice_details_td_power_tolls_data(
                fact, pol
            ),
            ...
            "energy_tolls": self.get_sub_component_invoice_details_td_energy_tolls_data(
                fact, pol
            ),
            "energy_charges": self.get_sub_component_invoice_details_td_energy_charges_data(  # noqa: E501
                fact, pol, energy_discount_BOE17_2021
            ),
            "other_concepts": self.get_sub_component_invoice_details_td_other_concepts_data(  # noqa: E501
                fact, pol
            ),
            ...
            "bo_social_2023": self.get_sub_component_invoice_details_td_bo_social_2023_data(  # noqa: E501
                fact, pol
            ),
            "generation": self.get_sub_component_invoice_details_td_generation_data(
                fact, pol
            ),
            "inductive": self.get_sub_component_invoice_details_td_inductive_data(
                fact, pol
            ),
            "capacitive": self.get_sub_component_invoice_details_td_capacitive_data(
                fact, pol
            ),
            "amount_total": fact.amount_total,
        }
    ...
    def get_sub_component_invoice_details_td(self, fact, pol, linies, lectures=None):
        ...
        if lectures is not None:
            for comptador_name, lectures_comptador in lectures.items():
                for lectura in lectures_comptador:
                    if lectura[0] == u"P1":
                        data = str(
                            datetime.strptime(lectura[4], "%d/%m/%Y").date()
                            + timedelta(days=1)
                        )
                        origin = _(u"estimada")
        ...
        for l in sorted_linies:  # noqa: E741
            ...
            if block in lines_data and l.name in lines_data[block]:
            ...
            lines_data[block]["origin"] = (
                readings[l.data_desde]
                if l.data_desde in readings
                else _(u"sense lectura")
            )
    ...
    def get_sub_component_invoice_details_td_power_discount_BOE17_2021_data(
        self, fact, pol
    ):
        ...
        discount_lines = [
            ...
            and l.invoice_line_id.product_id.code
            in ("DESC1721", "DESC1721POT", "DESC1721ENE")
    ...
    def get_sub_component_invoice_details_td_energy_discount_BOE17_2021_data(
        self, fact, pol
    ):
        is_index = is_indexed(fact)
        discount_lines = [
    ...
    def get_sub_component_invoice_details_td_power_tolls_data(self, fact, pol):
        ...
        data["showing_periods"] = self.get_matrix_show_periods(pol)
        data["dies"] = (
            int(atr_linies_potencia["P1"]["days"]) if "P1" in atr_linies_potencia else 0
        )
        data["dies_any"] = days_year
        return data

    def get_sub_component_invoice_details_td_power_charges_data(
        self, fact, pol, discount
    ):
        ...
        for l in fact.linies_potencia:  # noqa: E741
                ...
                l_count = Counter(
                    {
                        ...
                        "our_cargos": float(
                            l.quantity
                            * self.get_atr_price(fact, tarifa_elect_atr, l)
                            * l.multi
                        )
                        / float(days_year),
                        "multi": l.multi,
            ...
            l_name = l.name[:2]
            if l_name not in charges_lines_potencia.keys():
                if (
        ...
        data["showing_periods"] = self.get_matrix_show_periods(pol)
        data["dies"] = (
            int(charges_lines_potencia["P1"]["days"])
            if "P1" in charges_lines_potencia
            else 0
        )
        data["dies_any"] = days_year
    ...
    def get_component_invoice_details_info_td_data(self, fact, pol):
        ...
        data = {
            "has_autoconsum": te_autoconsum(fact, pol),
            "has_mag": has_mag,
            "preu_mitja_mag_darrer_mes": mag_info["mes_natural_anterior"][
                "preu_mig_mensual_ajust"
            ],
    ...
    def get_sub_component_invoice_details_td_energy_data(self, fact, pol, discount):
        ...
        linies_energia = self.get_real_energy_lines(fact, pol)
        no_generationkWh_lines = [
            l for l in linies_energia if "GkWh" not in l.name  # noqa: E741
        ]
        ...
        generationkwh_lines = [
            l for l in linies_energia if "GkWh" in l.name  # noqa: E741
        ]
    ...
    def get_sub_component_invoice_details_td_energy_charges_data(
        self, fact, pol, discount
    ):
        ...
        for l in linies_energia:  # noqa: E741
            ...
            l_name = l.name[:2]
            if l_name not in charges_lines_energy.keys():
                if (
                    l.data_desde >= BOE17_2021_dates["start"]
                    and l.data_desde <= BOE17_2021_dates["end"]
                    and l_name in discount
        ...
        for k, v in charges_lines_energy.items():
            d = (
                discount[k]["price_subtotal"]
                if discount["is_visible"] and k in discount
                else 0
            )
    ...
    def get_sub_component_invoice_details_td_excess_power_quarterhours(self, fact, pol):
        ...
        # after this date has a 31/30 factor depending on month lenght
        ...
        for excess_lines in excess_lines_data:
            ...
            for p in showing_periods:
                if p in excess_lines:
                    item = {}
                    item["kp"] = (
                        tariff.factors_k[p] if replace_kp else excess_lines[p]["extra"]
                    )
                    item["power_excess"] = excess_lines[p]["quantity"]
                    item["price_excess"] = excess_lines[p]["price_unit_multi"]
                    item["price_subtotal"] = excess_lines[p]["price_subtotal"]
    ...
    def get_sub_component_invoice_details_td_inductive_data(self, fact, pol):
        if is_6XTD(pol):
            linies_inductiva = [
                l for l in fact.linies_reactiva if l.name not in (u"P6")  # noqa: E741
            ]
        else:
    ...
    def get_sub_component_invoice_details_td_capacitive_data(self, fact, pol):
        linies_capacitiva = [
            l for l in fact.linies_reactiva if l.name in (u"P6")  # noqa: E741
        ]
    ...
    def get_component_amount_destination_td_data(self, fact, pol):
        ...
        pie_charges = (
            round(float(fact.total_cargos), 2) if "total_cargos" in fact else 0.0
        )
        ...
        # BOE17/2021 wrong calculations in the invoice charges needs
        # to remove twice the discount
        ...
        pie_energy = round(
            pie_total - pie_renting - pie_taxes - pie_tolls - pie_charges, 2
        )
        data = {
    ...
    def get_sub_component_energy_consumption_detail_meter_td_data(self, fact, pol, meter):
        ...
        data = {
            "name": meter.name,
            "showing_periods": self.get_matrix_show_periods(pol),
            "active": add_last_visible(
                active, [surplus, inductive, capacitive, maximeter]
            ),
    ...
    def adjust_readings_priority(self, adjust_readings_list):
        adjust_readings_list = [
            adjust for adjust in adjust_readings_list if adjust is not False
        ]
    ...
    def get_sub_component_energy_consumption_detail_td_active_data(
        self, fact, pol, meter
    ):
        (
            a,
    ...
    def get_sub_component_energy_consumption_detail_td_surplus_data(
        self, fact, pol, meter
    ):
        (
            a,
        ...
        if meter.name in lectures:
            data["is_visible"] = len(
                lectures[meter.name]
            ) > 0 and te_autoconsum_amb_excedents(fact, pol)
            for reading in lectures[meter.name]:
                data[reading[0]] = {
                    "initial": reading[1],
                    "final": reading[2],
    ...
    def get_sub_component_energy_consumption_detail_td_inductive_data(
        self, fact, pol, meter
    ):
        (
            a,
    ...
    def get_sub_component_energy_consumption_detail_td_capacitive_data(
        self, fact, pol, meter
    ):
        (
            a,
    ...
    def get_sub_component_energy_consumption_detail_td_maximeter_data(
        self, fact, pol, meter
    ):
        excess_lines = [
            l for l in fact.linia_ids if l.tipus == "exces_potencia"  # noqa: E741
    ...
    def get_sub_component_energy_consumption_detail_td_info_data(
        self, fact, pol, adjust_reason
    ):
        dies_factura = (
            datetime.strptime(fact.data_final, "%Y-%m-%d")
            - datetime.strptime(fact.data_inici, "%Y-%m-%d")
    ...
    def get_component_cnmc_comparator_qr_link_data(self, fact, pol):
        ...
        data = {
            ...
            "lang": fact.lang_partner.lower(),
            "link_qr": qr_data["url"]
            if max_potencies_demandades
            else "https://comparador.cnmc.gob.es/",
