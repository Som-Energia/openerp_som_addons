...
class SomL10nEsAeatMod347Report(osv.osv):
    ...
    def send_email_clients_import_over_limit(self, cursor, uid, report_ids, context=None):
        ...
        This report includes clients and providers with an annual
        invoice total over an specific limit.
        """
        if len(report_ids) != 1:
            return False
        else:
        ...
        if (
            not report
            or report.get("state") != "calculated"
            or len(partner_record_ids) == 0
        ):
            return False
        ...
        for partner_record_id in partner_record_ids:
            SomL10nEsAeatMod347Helper.send_email(
                cursor, uid, self, partner_record_id, email_params
            )
...
class SomL10nEsAeatMod347Partner(osv.osv):
    ...
    def send_annual_import_summary_email(
        self, cursor, uid, partner_record_ids, context=None
    ):
        """
        ...
        report_id = partner_record_obj.read(
            cursor, uid, partner_record_ids[0], ["report_id"]
        ).get("report_id", [None])[0]
        report = report_obj.read(cursor, uid, report_id)
        if (
            not report
            or report.get("state") != "calculated"
