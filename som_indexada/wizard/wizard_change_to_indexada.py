...
class WizardChangeToIndexada(osv.osv_memory):
    ...
    def calculate_new_pricelist(self, cursor, uid, polissa, change_type, context=None):
        ...
        if polissa.fiscal_position_id:
            ...
            "INSULAR" in polissa.llista_preu.name or "balears" in polissa.llista_preu.name
        ):
            location = "balears"
        else:
            location = "peninsula"
    ...
    def validate_polissa_can_change(
        self, cursor, uid, polissa, change_type, context=None
    ):
    ...
    def send_indexada_modcon_created_email(self, cursor, uid, polissa):
        ...
        if not email_from:
            email_from = account_obj.search(
                cursor, uid, [("email_id", "=", email_account_id)]
            )[0]
    ...
    def change_to_indexada(self, cursor, uid, ids, context=None):
        ...
        coefs = (
            self.calculate_k_d_coeficients(cursor, uid)
            if change_type == "from_period_to_index"
            else None
        )
        ...
        new_modcon_vals = modcon_obj.copy_data(cursor, uid, prev_modcon.id)[0]
        new_observacions = (u"* Modcon canvi a {}:\n " u"Nova tarifa comer: {}").format(
            CHANGE_AUX_VALUES[change_type]["comments"], new_pricelist.name
        )
        new_modcon_vals.update(
            {
                ...
                "mode_facturacio_generacio": CHANGE_AUX_VALUES[change_type][
                    "invoicing_type"
                ],
