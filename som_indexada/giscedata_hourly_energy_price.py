...
class GiscedataNextDaysEnergyPrice(osv.osv):
    ...
    def get_monthly_price_baleares(self, cursor, uid, tarifa, data_inici, day=None, context=None):
        ...
        num_days = calendar.monthrange(data_inici_dt.year, data_inici_dt.month)[1]
        ...
        periode_ids = periode_obj.search(cursor, uid, [('tarifa', '=', tarifa['id']), ('tipus', '=', 'te')],
                                         context=ctx)
        ...
        margins_pl = pl_config_data.get(tarifa['name'])  ## TODO: Veure com ho fem per balears i canaries
        ...
        if margins_pl:
            ...
            k = pricelist_obj.price_get(cursor, uid, [margins_pl], k_item, 1, context=ctx)[margins_pl]
            ...
            d = pricelist_obj.price_get(cursor, uid, [margins_pl], d_item, 1, context=ctx)[margins_pl]
        ...
        sphdem = SphdemDD_BALEARES('A1_%(filename)s_%(postfix)s' % locals(), esios_token)  # [€/MWh]
        ...
        si = SIFree('A1_sifree_%(postfix)s' % locals(), esios_token)  # [€/MWh]
    ...
    def get_monthly_price_canaries(self, cursor, uid, tarifa, data_inici, day=None, context=None):
        ...
        sphdem = SphdemDD_CANARIAS('A1_%(filename)s_%(postfix)s' % locals(), esios_token)  # [€/MWh]
    ...
    def get_monthly_price(self, cursor, uid, tarifa, data_inici, day=None, context=None):
        """
        ...
        try:
            # COMPODEM
            compodem = MonthlyCompodem('A1_monthlycompodem_%(postfix)s' % locals(), esios_token)
            rad3 = compodem.get_component("RAD3")
            bs3 = compodem.get_component("BS3")
        except REECoeficientsNotFound as e:
            rad3 = Component(datetime.strptime(data_inici, "%Y-%m-%d"))
            bs3 = Component(datetime.strptime(data_inici, "%Y-%m-%d"))

        try:
            # SOBRECOSTOS
            sobrecostos = Prdemcad('C2_prdemcad_%(postfix)s' % locals(), esios_token)
        except REECoeficientsNotFound as e:
            sobrecostos = Component(datetime.strptime(data_inici, "%Y-%m-%d"))
        ...
        ## AJOM APLICATION
        aplica_ajom = False
        if ((data_inici_dt.year >= 2022 and data_inici_dt.month >= 6) or
                (data_inici_dt.year == 2023 and data_inici_dt.month < 6)):
            aplica_ajom = True
        ...
        for periode in periode_obj.browse(cursor, uid, periode_ids, context=context):
            if periode.agrupat_amb:
                ...
                prod = periode.product_id.id
            pc[periode.name] = pricelist_obj.price_get(cursor, uid, [pc_pl], prod, 1, context=ctx)[pc_pl]
        versions = {
            data_inici: {
                'pc': pc,
                'atr': te,
        ...
        if aplica_ajom:
            if day is None:
                ...
                data_final_ajom = day
            ajom = facturador_obj.get_ajom_prices_from_omie(cursor, uid, data_inici, data_final_ajom)
        ...
        if omie_pl:
            ...
            omie = pricelist_obj.price_get(cursor, uid, [omie_pl], omie_item, 1, context=ctx)[omie_pl]
        ...
        ## PERD BY TARIFF
        perdfname = Tarifa.perdclass.name
        perdues = perdues = Tarifa.perdclass('A1_%(perdfname)s_%(ym)s01_%(ym)s%(num_days)s' % locals(), esios_token)
