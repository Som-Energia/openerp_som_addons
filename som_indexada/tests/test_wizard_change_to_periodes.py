...
class TestChangeToPeriodes(TestSwitchingImport):
    ...
    def open_polissa(self, xml_id, indexed=False):
        ...
        polissa_obj.send_signal(
            self.cursor, self.uid, [polissa_id], ["validar", "contracte"]
        )
    ...
    def test_change_to_periodes_modcon_pendent_polissa(self, mocked_send_mail):
        ...
        self.assertEqual(
            ...
            u"Pòlissa 0018 already has a pending modcon",
        )
    ...
    def test_change_to_periodes_already_period_polissa(self):
        ...
        with self.assertRaises(indexada_exceptions.PolissaAlreadyPeriod) as error:
            wiz_o.change_to_indexada(self.cursor, self.uid, [wiz_id], context=context)
        self.assertEqual(
            error.exception.to_dict()["error"], u"Pòlissa 0018 already period"
        )
    ...
    def test_change_to_periodes_tariff_code_not_supported(self):
        ...
        self.assertEqual(
            error.exception.to_dict()["error"],
            u"Change with tariff code 2.0A not supported",
        )
    ...
    def test_change_to_periodes_atr_en_curs_polissa(self):
        ...
        with self.assertRaises(indexada_exceptions.PolissaSimultaneousATR) as error:
            wiz_o.change_to_indexada(self.cursor, self.uid, [wiz_id], context=context)
        self.assertEqual(
            error.exception.to_dict()["error"], u"Pòlissa 0018 with simultaneous ATR"
        )
        ...
        self.assertEquals(
            modcon_act,
            {
                ...
                "data_final": datetime.strftime(
                    date.today() + timedelta(days=365), "%Y-%m-%d"
                ),
                "mode_facturacio": "atr",
