...
class TestsFacturacioValidation(testing.OOTestCase):
    ...
    def test_check_missing_days(self):
        ...
        fact_obj.write(cursor, uid, factures[0], {
            'lot_facturacio': lot_id,
            'data_final': start_date + timedelta(days=2)
        })
        ...
        facts = fact_obj.search(cursor, uid, [
            ('polissa_id', '=', fact.polissa_id.id),
            ('id', '!=', factures[0])
        ])

        fact_obj.write(cursor, uid, facts, {'data_inici': '1000-01-01', 'data_final': '1000-02-02'},
                       context={"skip_cnt_llista_preu_compatible": True})

        fact.polissa_id.send_signal([
            'validar', 'contracte'
        ])

        crear_modcon(self.openerp.pool,
                     cursor,
                     uid,
                     fact.polissa_id.id,
                     {},
        ...
        fact_obj.write(cursor, uid, factures[1], {'data_inici': '9300-01-01', 'data_final': '9300-02-02'},
                       context={"skip_cnt_llista_preu_compatible": True})
