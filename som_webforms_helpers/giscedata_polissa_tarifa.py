...
class GiscedataPolissaTarifa(osv.osv):
    ...
    def get_bo_social_price(
        ...
        bs_id = imd_obj.get_object_reference(
            cursor, uid, "som_polissa_soci", "bosocial_BS01"
        )[1]
        prod = prod_obj.browse(cursor, uid, bs_id)

        price = pricelist.price_get(bs_id, 1, 1, context)[pricelist.id]
        if with_taxes:
            price = prod.add_taxes(
                price, fiscal_position, direccio_pagament=None, titular=None
            )
    ...
    def get_comptador_price(
        ...
        default_comptador_id = prod_obj.search(
            cursor, uid, [("default_code", "=", "ALQ01")]
        )
    ...
    def _get_general_price_version_list(
        self, cursor, uid, pricelist_list, date_from, date_to
    ):
        """
        Some prices are calculated using other pricelists different from somenergia pricelists
        """  # noqa: E501
        pricelist_general = [
            item for item in pricelist_list if item.name == "TARIFAS ELECTRICIDAD"
        ]
    ...
    def _get_default_fiscal_position_id(self, cursor, uid, date_from, date_to):
        ...
        prop_id = prop_obj.search(
            cursor,
            uid,
            [("name", "=", "property_account_position"), ("res_id", "=", False)],
        )
    ...
    def _get_fiscal_position_igic(self, cursor, uid, date_from, date_to, home):
        ...
        for (
            ...
            end_date_igic,
            home_igic_tax_id,
            industrial_igic_tax_id,
        ) in eval(taxes_ids_by_period):
            if end_date_igic >= date_from and start_date_igic <= date_to:
                ...
                fiscal_position = fp_obj.browse(cursor, uid, fiscal_position_id)
                fiscal_position_data.append(
                    (start_date_igic, end_date_igic, fiscal_position)
                )
    ...
    def _get_reactiva_cosfi_prices(self, cursor, uid, pricelist, context=None):
        ...
        for cosfi in self._cosfi:
            price = fact_obj.calc_cosfi_price(
                cursor, uid, cosfi[0], pricelist.id, context
            )
            price_cosfi.append({"cosfi": cosfi, "price": price})

        return price_cosfi

    def _get_reactiva_price(
        self, cursor, uid, general_price_version_list, price_version, context
    ):
        """
        ...
        if fiscal_positions:
            ...
            for pl in pricelists:
                for fp_date_start, fp_date_end, fp in ordered_fiscal_position:
                    if (
                        fp_date_end >= pl.date_start
                        or pl.date_end
                        or fp_date_start <= pl.date_end
                    ):
                        fp = dict(
                            fp=fp, fp_date_start=fp_date_start, fp_date_end=fp_date_end
                        )
                    ...
                    for tipus_product in tipus_products:
                        ...
                        if tipus == "tr":
                            ...
                            for (
                                ...
                                if cosfi_desc not in preus[self._desc_tipus["tr"]]:
                                    preus[self._desc_tipus["tr"]][cosfi_desc] = {}

                                preus[self._desc_tipus["tr"]][cosfi_desc][cosfi_value] = {
                                    "value": round(
                                        cosfi_price, config.get("price_accuracy", 6)
                                    ),
                                    "unit": "€/{}".format(unit),
                                }
                        else:
                            ...
                            preus[self._desc_tipus[tipus]][name] = {
                                ...
                                "unit": "€/{}".format(
                                    uom.name if uom.name != "PCE" else "kWh"
                                ),
                            }
    ...
    def _get_dades_modcontractuals(self, modcon_data):
        ...
        """  # noqa: E501
        ordered_modcon_data = sorted(
            modcon_data, key=lambda element: (element[3], element[4])
        )
    ...
    def get_tariff_prices_by_contract_id(
        ...
        try:
            ...
            for (
                ...
                prices_by_contract[str(tariff_id)]["current"] = price_by_date_range[
                    "current"
                ]
                prices_by_contract[str(tariff_id)]["history"].extend(
                    price_by_date_range["history"]
                )
