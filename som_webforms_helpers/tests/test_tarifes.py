...
class tarifes_tests(testing.OOTestCase):
    ...
    def test__get_tariff_prices__invalid_date(self):
        ...
        with Transaction().start(self.database) as txn:
            ...
            self.assertEqual(
                ctx.exception.to_dict()["error"], "Tariff pricelist not found"
            )
    ...
    def test__get_tariff_prices_by_contract_id__no_titular(self):
        ...
        with Transaction().start(self.database) as txn:
            ...
            result = model.get_tariff_prices_by_contract_id_www(
                cursor, uid, polissa_id, False
            )
    ...
    def test__get_tariff_prices__valid_date_range_date_tariff_into_range(self):
        ...
        self.assertTrue(
            (not date_end or date_from <= date_end)
    ...
    def test__get_fiscal_position_igic__home(self):
        with Transaction().start(self.database) as txn:
            ...
            expected_result = [
                ("2021-06-01", "2999-12-31", afp_obj.browse(cursor, uid, 33))
            ]
    ...
    def test__combine_pricelist_fiscal_position__pricelist_data(self):
        with Transaction().start(self.database) as txn:
            ...
            pricelists = [
                pricelist for pricelist in pplv_obj.browse(cursor, uid, pricelists_ids)
            ]
            ...
            result = tariff_obj._combine_pricelist_fiscal_position(
                pricelists, fiscal_positions
            )
