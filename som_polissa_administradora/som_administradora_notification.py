...
class SomAdministradoraModification(osv.osv):
    ...
    def _ff_info_search(self, cursor, uid, obj, name, args, context=None):
        ...
        info_ids = modi_obj.search(cursor, uid, [
            ('info', 'ilike', '%{}%'.format(search_str))
        ])
        pol_ids = modi_obj.search(cursor, uid, [
            ('polissa_id.name', 'ilike', '%{}%'.format(search_str))
        ])
        old_admin_ids = modi_obj.search(cursor, uid, [
            ('old_administradora.name', 'ilike', '%{}%'.format(search_str))
        ])
        new_admin_ids = modi_obj.search(cursor, uid, [
            ('new_administradora.name', 'ilike', '%{}%'.format(search_str))
        ])
    ...
    _columns = {
        'name': fields.function(_ff_info,
                                method=True, string="Nom",
                                type="string",
                                fnct_search=_ff_info_search),
        'claimant': fields.many2one(
            'res.partner',
            u'Sol·licitant',
            readonly=True),
        ...
        'old_administradora': fields.many2one(
            'res.partner',
            'Administradora desassignada',
            readonly=True),
        'new_administradora': fields.many2one(
            'res.partner',
            'Administradora assignada',
            readonly=True),
        ...
        'error': fields.boolean(
            u'Indica si hi ha hagut error o no')
    }
...
class SomAdministradoraNotification(osv.osv):
    ...
    def send_email(self, cursor, uid, ids, context=None):
        ...
        ctx = {'active_ids': [ids], 'active_id': ids,
               'template_id': template_id, 'src_rec_ids': [ids], 'src_rec_id': ids,
               'src_model': 'som.admin.notification', 'from': mail_from,
