...
class WizardModifyOVAdmin(osv.osv_memory):
    ...
    def create_admin_notification_modification(self, cursor, uid, partner_id,
        ...
        if partner_id not in notification_receptor.keys():
            notification_receptor[partner_id] = admin_noti_obj.create(cursor, uid, {
                "receptor": partner_id, "pending_notification": True,
    ...
    def action_assignar_administradora(self, cursor, uid, ids, context=None):
        ...
        new_administradora_name = partner_obj.read(
            cursor, uid, new_administradora_id, ['name'])['name']
        ...
        for pol in polisses:
            ...
            if error:
                error_msg = u"- S'ha produït un error al assignar l'administradora " + \
                    u"per la pòlissa {}: \"{}\"\n"
                ...
                if old_administradora_id is not None:
                    old_administradora_name = partner_obj.read(
                        cursor, uid, old_administradora_id, ['name'])['name']
                    msg = u"- S'ha sustituit el partner '{}' (ID {}) pel partner " + \
                        u"'{}' (ID {}) com a administradora OV a la pòlissa {}\n"
                    msg = msg.format(
                        old_administradora_name, old_administradora_id,
                        new_administradora_name, new_administradora_id, pol.name)
                else:
                    msg = u"- S'ha assignat el partner '{}' (ID {}) " + \
    ...
    _columns = {
        'new_administradora': fields.many2one(
            'res.partner', 'Nova Administradora OV'
        ),
        'claimant': fields.many2one(
            'res.partner', 'Sol·licitant'
        ),
