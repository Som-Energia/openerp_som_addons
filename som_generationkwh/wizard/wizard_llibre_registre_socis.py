...
class WizardLlibreRegistreSocis(osv.osv_memory):
    ...
    def generate_report(self, cursor, uid, ids, context=None):
        ...
        j_pool = ProgressJobsPool(logger_description="reports")
        aw = AutoWorker(queue="reports", default_result_ttl=24 * 3600, max_procs=1)
        j_pool.add_job(self.generate_one_report(cursor, uid, ids, context))
        aw.work()
        return {}

    @job(queue="reports", timeout=3000)
    ...
    def get_soci_values(self, cursor, uid, soci, context=None):
        ...
        data = soci_obj.read(cursor, uid, soci, [
            'ref', 'name', 'vat', 'www_email', 'www_street', 'www_zip', 'www_provincia', 'date',
            'data_baixa_soci', 'www_municipi'
        ])
        singles_soci_values = {
            'tipus': 'Consumidor',
            'num_soci': data['ref'],
    ...
    def get_aportacions_voluntaries_values(self, cursor, uid, soci, context=None):
        ...
        for inv in inv_list:
            data = inv_obj.read(cursor, uid, inv, [
                'purchase_date', 'last_effective_date', 'last_effective_date', 'nshares',
                'amortized_amount'
            ])
            if (
                data['purchase_date']
