...
class InvestmentActions(ErpWrapper):
    ...
    def create_from_transfer(
            self, cursor, uid, investment_id, new_partner_id,
    ...
    def create_divestment_invoice(self, cursor, uid,
        ...
        if irpf_amount_current_year:
            ...
            Investment.irpfRetentionLine(
                cursor, uid, investment, irpf_amount_current_year,
                invoice_id, isodate(date_invoice), investmentMemento
            )
        if irpf_amount:
            irpf_amount = round(irpf_amount, 2)
            Investment.irpfRetentionLine(
                cursor, uid, investment, irpf_amount, invoice_id,
                isodate(date_invoice) - relativedelta(years=1),
                investmentMemento
            )
...
class GenerationkwhActions(InvestmentActions):
    ...
    def create_from_transfer(
        ...
        if (
                old_investment['amortized_amount']
                >= old_investment['nshares'] * gkwh.shareValue):
        ...
        GenerationkwhInvestment.move_line_when_tranfer(
            ...
            new_partner.property_account_gkwh.id, amount,
            transmission_date
        )
...
class AportacionsActions(InvestmentActions):
    ...
    def create_from_form(
            self, cursor, uid, partner_id, order_date, amount_in_euros,
            ip, iban, emission=None, context=None):
        ...
        GenerationkwhInvestment.send_mail(
            cursor, uid, investment_id, 'generationkwh.investment',
    ...
    def create_interest_invoice(self, cursor, uid,
        ...
        if (
                investment.first_effective_date
                and investment.first_effective_date > vals['date_start']):
