...
class TestImportF1Som(TestImportF1):
    def test_validation_3035S_ok(self):
        ...
        err_template_obj = self.openerp.pool.get(
            "giscedata.facturacio.switching.error.template"
        )
        with Transaction().start(self.database) as txn:
            cursor = txn.cursor
            uid = txn.user
            err_template_id = imd_obj.get_object_reference(
                cursor, uid, "som_facturacio_switching", "error_phase_3_code_S35"
            )[1]
            err_template_obj.write(cursor, uid, err_template_id, {"level": "critical"})

            self.set_comer_and_distri_for_polissa(
                txn, "polissa_tarifa_018", "4321", "1234"
            )
            self.canvi_potencies(
                txn, "polissa_tarifa_018", 5.75, {"P1": 4.75, "P2": 5.75}
            )
            self.activar_polissa(txn, polissa_ref="polissa_tarifa_018")
            xml_path = get_module_resource(
                "giscedata_facturacio_switching", "tests", "fixtures", "test_20TD.xml"
            )
        ...
        with Transaction().start(self.database) as txn:
            ...
            with open(xml_path, "r") as f:
                ...
                xml_file = xml_file.replace(
                    "<Lectura>330.00</Lectura>", "<Lectura>430.00</Lectura>"
                )
    ...
    def test_validation_3035S_ok_within_tolerance(self):
        ...
        with Transaction().start(self.database) as txn:
            ...
            with open(xml_path, "r") as f:
                xml_file = f.read()
                xml_file = xml_file.replace(
                    "<Lectura>330.00</Lectura>", "<Lectura>329.00</Lectura>"
                )
                xml_file = xml_file.replace(
                    "<Lectura>220.00</Lectura>", "<Lectura>221.00</Lectura>"
                )
    ...
    def test_validation_3035S_error_one_within_tolerance(self):
        ...
        with Transaction().start(self.database) as txn:
            ...
            with open(xml_path, "r") as f:
                ...
                xml_file = xml_file.replace(
                    "<Lectura>220.00</Lectura>", "<Lectura>222.00</Lectura>"
                )
