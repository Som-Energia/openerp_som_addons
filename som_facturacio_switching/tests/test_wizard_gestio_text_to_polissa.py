...
class TestWizardGestioTextToPolissa(testing.OOTestCaseWithCursor):
    ...
    def test_get_polissa_ids_from_f1_pol_activa(self):
        ...
        other_pols_id = pol_o.search(self.cursor, self.uid, [(
            'cups', '=', pol.cups.id), ('state', '!=', 'activa')])
        ...
        fecha_factura_hasta = datetime.strptime(
            fecha_factura_desde, '%Y-%m-%d') + timedelta(days=15)
