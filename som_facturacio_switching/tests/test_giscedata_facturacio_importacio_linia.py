...
class TestGiscedataFacturacioImportacioLinia(testing.OOTestCaseWithCursor):
    def setUp(self):
        ...
        f1_ids = self.f1_obj.search(self.cursor, self.uid, [
                                    ('invoice_number_text', '=', False)], limit=2)
        self.f1_obj.write(self.cursor, self.uid, f1_ids[0], {
                          'invoice_number_text': 'sample_origin1'})
        self.f1_obj.write(self.cursor, self.uid, f1_ids[1], {
                          'invoice_number_text': 'sample_origin2'})
    ...
    def test_search_withPercentage_active(self):
        ...
        f1_ids = self.f1_obj.search(self.cursor, self.uid, [(
            'invoice_number_text', 'ilike', 'sample_origin%')])
    ...
    def test_search_exactExist_active(self):
        ...
        f1_ids = self.f1_obj.search(self.cursor, self.uid, [(
            'invoice_number_text', 'ilike', 'sample_origin1')])
    ...
    def test_do_reimport_f1_select_one_f1(self, mock_function):
        ...
        data = {'error_codes': [
            {'code': '2999', 'text': 'no file in gridfs'}], 'days_to_check': 30}
    ...
    def test_do_reimport_f1_two_f1_same_cups(self, mock_function):
        ...
        self.f1_obj.write(self.cursor, self.uid, f1_id, {
                          'fecha_factura_desde': '2022-02-02'})
        self.f1_obj.write(self.cursor, self.uid, f2_id, {
                          'fecha_factura_desde': '2022-01-02'})
        data = {'error_codes': [
            {'code': '2999', 'text': 'no file in gridfs'}], 'days_to_check': 30}
        self.f1_obj.do_reimport_f1(self.cursor, self.uid, data=data, context=None)

        mock_function.assert_called_with(
            self.cursor, self.uid, [f2_id, f1_id], context={})
    ...
    def test_do_reimport_f1_two_f1_different_cups(self, mock_function):
        ...
        self.f1_obj.write(self.cursor, self.uid, [f1_id, f2_id], vals)
        self.f1_obj.write(self.cursor, self.uid, f1_id, {
                          'fecha_factura_desde': '2022-01-02', 'cups_id': cups1_id})
        self.f1_obj.write(self.cursor, self.uid, f2_id, {
                          'fecha_factura_desde': '2022-02-02', 'cups_id': cups2_id})
        data = {'error_codes': [
            {'code': '2999', 'text': 'no file in gridfs'}], 'days_to_check': 30}
        self.f1_obj.do_reimport_f1(self.cursor, self.uid, data=data, context=None)

        self.assertEqual(mock_function.call_count, 2)
        mock_function.assert_has_calls([call(self.cursor, self.uid, [f1_id], context={}), call(
            self.cursor, self.uid, [f2_id], context={})])
