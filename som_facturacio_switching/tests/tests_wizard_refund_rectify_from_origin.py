...
class TestRefundRectifyFromOrigin(testing.OOTestCase):
    ...
    @mock.patch.object(wizard.wizard_refund_rectify_from_origin.WizardRefundRectifyFromOrigin,
    ...
    def test_refund_rectify_by_origin_refundOne(self, mock_lectures, mock_delete, mock_refund):
        ...
        fact_obj.write(cursor, uid, f_cli_ids[0], {
                       'state': 'open', 'polissa_id': fact_info['polissa_id'][0], 'cups_id': fact_info['cups_id'][0]})
        ...
        mock_lectures.assert_called_with(
            cursor, uid, [wiz_id], fact_info['polissa_id'][0], fact_info['data_inici'], fact_info['data_final'], ctx)
    ...
    def test_get_factures_client_by_dates_toRefundOne(self):
        ...
        result = wiz_obj.get_factures_client_by_dates(
            cursor, uid, wiz_id, fact_info['polissa_id'][0], fact_info['data_inici'], fact_info['data_final'],
            context=ctx
        )
