...
class WizardRefundRectifyFromOrigin(osv.osv_memory):
    ...
    def send_polissa_mail(self, cursor, uid, ids, pol_id, is_positive_amount, context):
        ...
        plantilla = (
            wiz.email_template_to_pay
            if is_positive_amount
            else wiz.email_template_to_refund
        )
    ...
    def delete_draft_invoices_if_needed(
        self, cursor, uid, fres_resultat, f_ids, context={}
    ):
        msg = []
        fact_obj = self.pool.get("giscedata.facturacio.factura")
    ...
    def open_group_invoices(
        ...
        if wiz.amount_total == 0:
            ...
            wiz_remesa_id = wiz_remesar_o.create(
                cursor,
                uid,
                {"tipus": "payable", "order": payment_order_id},
                context=remesar_ctx,
            )
            ...
            return True, "S'han agrupat les factures, l'import agrupat és {}".format(
                total_import
            )
    ...
    def add_f1_observation(self, cursor, uid, f1_id, text, context):
        ...
        f1_obj.write(
            cursor, uid, f1_id, {"user_observations": "{}\n{}".format(text, obs)}
        )

    def open_polissa_invoices_send_mail(
        self, cursor, uid, ids, facts_by_polissa, context={}
    ):
        ...
        for pol_name, facts_created in facts_by_polissa.items():
            ...
            if wiz.actions != "draft":
                ...
                msg.append(
                    "S'han obert les factures de la pòlissa {}. {}".format(
                        pol_name, msg_open
                    )
                )
            if res and wiz.actions == "open-group-order-send":
                ...
                self.send_polissa_mail(
                    cursor, uid, ids, pol_id, is_positive_amount, context
                )
                msg.append(
    ...
    def refund_rectify_by_origin(self, cursor, uid, ids, context={}):
        ...
        if wiz.actions == "open-group-order-send" and not (
            ...
            raise osv.except_osv(
                _("Error"), _("Per enviar el correu cal indicar les plantilles")
            )
        if (
            wiz.email_template_to_pay
            and not wiz.email_template_to_pay.enforce_from_account
        ):
            raise osv.except_osv(
                _("Error"),
        ...
        if (
            wiz.email_template_to_refund
            and not wiz.email_template_to_refund.enforce_from_account
        ):
            raise osv.except_osv(
                _("Error"),
                _(
        ...
        if wiz.actions in ["open-group-order-send", "open-group-order"] and not wiz.order:
            raise osv.except_osv(
                _("Error"),
                _("Per remesar les factures a pagar cal una ordre de pagament"),
            )
        ...
        for _id in sorted_ids:
            ...
            try:
                if f1.type_factura != "R":
                    ...
                    fact_csv_result.append(
                        [origen, pol_name, "F1 no és tipus rectificatiu"]
                    )
                    continue
                ...
                if msg_get:
                    msg.append(
                        "Pòlissa {} per l'F1 amb origen {}: {}".format(
    ...
    def _show_invoices(self, cursor, uid, ids, context=None):
        fact_ids = self.read(cursor, uid, ids[0], ["facts_generades"])[0][
            "facts_generades"
        ]
