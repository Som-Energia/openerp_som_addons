...
class WizardDeleteReimport2001F1(osv.osv_memory):
    ...
    def get_duplicated_f1(self, cursor, uid, ids, f1_id, origin, distribuidora_id, context=None):
        ...
        duplicated_f1_ids = f1_obj.search(cursor, uid,
                                          [('invoice_number_text', '=', origin), ('distribuidora_id',
                                                                                  '=', distribuidora_id), ('id', '!=', f1_id)]
                                          )
        ...
        for f1 in f1_obj.browse(cursor, uid, duplicated_f1_ids):
            if f1.state == 'valid' and (not f1.error_ids or '2002' not in [x.name for x in f1.error_ids]):
                error_list.append(
                    "Per l'origen {} hi ha un F1 correcte sense warning 2002, ID: {}".format(origin, f1.id))
            else:
                delete_ids.append(f1.id)
        if error_list:
            error_list.append("Per l'origen {} es podrien esborrar els F1 ns {}".format(
                origin, str(delete_ids)))
    ...
    def delete_reimport(self, cursor, uid, ids, context=None):
        ...
        for f1 in f1_obj.browse(cursor, uid, selected_ids):
            ...
            if f1.state != 'erroni':
                error_msg.append(u"F1 amb origen {} no és erroni".format(
                    f1.invoice_number_text))
                continue

            if not f1.error_ids or '2001' not in [x.name for x in f1.error_ids]:
                error_msg.append(u"F1 amb origen {} no té error 2001".format(
                    f1.invoice_number_text))
                continue

            no_draft_fact = fact_obj.search(cursor, uid, [
                ('origin', '=', f1.invoice_number_text), ('partner_id',
                                                          '=', f1.distribuidora_id.id),
                ('type', 'in', ['in_invoice', 'in_refund']), ('state', '!=', 'draft')
            ])

            if no_draft_fact:
                error_msg.append(u"F1 amb origen {} té una factura de proveïdor no en esborrany".format(
                    f1.invoice_number_text))
                continue

            can_continue, result = self.get_duplicated_f1(
                cursor, uid, ids, f1.id, f1.invoice_number_text, f1.distribuidora_id.id)
        ...
        if f1_to_reimport:
            ...
            wiz_reimport_f1_obj.sub_action_importar_f1(
                cursor, uid, [wiz_reimport_id], context=ctx)
