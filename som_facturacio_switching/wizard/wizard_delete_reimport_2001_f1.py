...
class WizardDeleteReimport2001F1(osv.osv_memory):
    ...
    def get_duplicated_f1(
        self, cursor, uid, ids, f1_id, origin, distribuidora_id, context=None
    ):
        f1_obj = self.pool.get("giscedata.facturacio.importacio.linia")
        duplicated_f1_ids = f1_obj.search(
            cursor,
            uid,
            [
        ...
        if error_list:
            error_list.append(
                "Per l'origen {} es podrien esborrar els F1 ns {}".format(
    ...
    def delete_reimport(self, cursor, uid, ids, context=None):
        ...
        for f1 in f1_obj.browse(cursor, uid, selected_ids):
            ...
            if f1.state != "erroni":
                error_msg.append(
                    u"F1 amb origen {} no és erroni".format(f1.invoice_number_text)
                )
                continue

            if not f1.error_ids or "2001" not in [x.name for x in f1.error_ids]:
                error_msg.append(
                    u"F1 amb origen {} no té error 2001".format(f1.invoice_number_text)
                )
                continue

            no_draft_fact = fact_obj.search(
                cursor,
                uid,
                [
                    ("origin", "=", f1.invoice_number_text),
                    ("partner_id", "=", f1.distribuidora_id.id),
            ...
            if not can_continue:
                ...
                draft_fact_ids = fact_obj.search(
                    cursor,
                    uid,
                    [
                        ("origin", "=", f1.invoice_number_text),
