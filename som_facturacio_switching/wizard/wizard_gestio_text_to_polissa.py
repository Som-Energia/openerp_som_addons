    ...
    def get_polisses_ids(self, cursor, uid, ids, context=None):
        ...
        if from_model == "giscedata.facturacio.importacio.linia":
            ...
            for _id in active_ids:
                ...
                pol_id = polissa_o.search(
                    cursor, uid, search_params, context={"active_test": False}
                )
                if len(pol_id) > 1:
                    raise osv.except_osv(
                        _("Error!"),
                        _("Trobades més d'una pòlissa per l'F1 amb cups {}").format(f1.cups_text),
                    )
                if not pol_id:
                    raise osv.except_osv(
                        _("Error!"),
                        _("No s'ha trobat pòlissa per l'F1 amb cups {}").format(f1.cups_text),
                    )
