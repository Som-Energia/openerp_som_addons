    ...
    def check_consum_lectures_facturat_incoherent(self, cursor, uid, line, f1, parameters=None):
        ...
        for fact_xml in f1.facturas_atr:
            ...
            if fact_xml.datos_factura.fecha_desde_factura:
                ...
                context = {'date': fact_xml.datos_factura.fecha_hasta_factura,
                           'from_f1': True, 'avoid_error': True}
                polissa_vals = polissa_obj.read(cursor, uid, polissa_id, [
                                                'contract_type'], context=context)
            ...
            for c in consume_lect_period:
                period = c['period']
                consume_by_period[period] = consume_by_period.get(
                    period, 0.0) + c['total_consum']
