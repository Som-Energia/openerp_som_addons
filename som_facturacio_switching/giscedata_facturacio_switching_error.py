...
class GiscedataFacturacioSwitchingValidator(osv.osv):
    ...
    def check_consum_lectures_facturat_incoherent(  # noqa: C901
        self, cursor, uid, line, f1, parameters=None
    ):
        if not parameters:
            parameters = {}
        errors = []
        ...
        for fact_xml in f1.facturas_atr:
            ...
            if fact_xml.datos_factura.tipo_factura in ("A", "R"):
                refunded_origin = (
                    fact_xml.datos_factura.codigo_factura_rectificada_anulada
                )
                fact_refunded = fact_obj.search(
                    cursor,
                    uid,
                    [
            ...
            if fact_xml.datos_factura.fecha_desde_factura:
                ...
                polissa_vals = polissa_obj.read(
                    cursor, uid, polissa_id, ["contract_type"], context=context
                )
            ...
            for c in consume_lect_period:
                period = c["period"]
                consume_by_period[period] = (
                    consume_by_period.get(period, 0.0) + c["total_consum"]
                )
            ...
            for period, cons_fact in consums_facturats.items():
                if (
                    abs(round(consume_by_period[period], 2) - round(cons_fact, 2))
                    > tolerance_by_period
                ):
                    errors.append(
                        {
    ...
    def get_consume_and_period_by_meter(self, cursor, uid, compt_xml):
        ...
        for lect_xml in compt_xml.get_lectures(tipus=["A"]):
            if lect_xml.periode:
                lect_hasta = float(
                    lect_xml.lectura_hasta.lectura_data.Lectura.text.strip()
                )
                lect_desde = float(
                    lect_xml.lectura_desde.lectura_data.Lectura.text.strip()
                )
                ...
                period_consum.append(
                    {"period": lect_xml.periode, "total_consum": consume}
                )
