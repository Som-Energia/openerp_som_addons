...
class FacturacioExtra(osv.osv):
    ...
    def _ff_origin_number_search(self, cursor, uid, obj, field_name, arg, context=None):
        if not arg:
            ...
            if arg[0][2]:
                ...
                extra_ids = (
                    linia_extra_obj.q(cursor, uid)
                    .read(["extra_id"])
                    .where([("linia_id.invoice_number_text", "=", origin_number)])
                )
    ...
    def _ff_origin_number(self, cursor, uid, ids, field_name, arg, context=None):
        ...
        for id in ids:
            fact_number = False
            linia_id = (
                linia_extra_obj.q(cursor, uid).read(["linia_id"]).where([("extra_id", "=", id)])
            )
    ...
    _columns = {
        ...
        "data_baixa_polissa": fields.related(
            "polissa_id", "data_baixa", type="date", string="Data baixa pòlissa", readonly=True
        ),
